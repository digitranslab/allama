...
<Note>
  This tutorial does not cover how to expose your Allama instance to the public internet.

- If you're running Allama locally, you'll need use a tunneling service like [ngrok](https://ngrok.com/) to receive webhook requests from the public internet.
- If you're running Allama on AWS Fargate, Allama is automatically exposed to the public internet via an Application Load Balancer.
</Note>
...
Every Allama workflow comes with a single `Trigger` action.
...
```bash
WEBHOOK_URL=https://<allama-public-url>/api/webhooks/<workflow-id>/<webhook-secret>
```
...
3. Add the following header to every webhook request:
   ...
   ```http
   x-allama-api-key: <your-api-key>
   ```

If a request omits the header or uses a revoked key, Allama rejects it with an
...
Allama compares the requester's IP address against the allowlist. Requests
...
From here you can enter a recurring schedule with a timeout and offset. Allama defaults to using UTC time, so it's convenient to use `offset` to work with timezone differences.
...
This is used to verify that the webhook was received and processed by Allama.
...
<Note>
  ...
  [GitHub](https://github.com/digitranslab/allama/issues) and we'll add support
  for it.
</Note>

Some vendors have more complex webhook verification requirements.
To support this, Allama supports vendor-specific webhook verification.
...
Allama's webhook parser supports the following `Content-Type` headers:
...
<Note>
  ...
  value. If no `Content-Type` header is set, Allama defaults to
  `application/json`.
</Note>
...
<Accordion title="Step-by-step guide" icon="play">
  <Steps>
    ...
    <Step title="Receive webhook in Allama">
      Allama looks at the `Content-Type` header to determine how to parse the webhook payload.
      In this case, we're using `application/x-ndjson`, so Allama will parse the webhook payload as NDJSON.
