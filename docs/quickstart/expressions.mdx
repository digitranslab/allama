...
Allama supports the following expression contexts:
...
<Accordion title="Webhook example" icon="globe">
  ...
  ```bash
  export WEBHOOK_URL=https://allama.com/api/webhooks/<webhook_id>/<secret>
...
Allama comes with a built-in secrets manager.
...
Allama will automatically replace the expression with the secret value at runtime.
...
Allama supports inline functions in the `FN` context.
