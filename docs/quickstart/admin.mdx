---
title: User admin
description: Log into Allama and invite your team.
icon: user
---

<Note>
  This tutorial assumes you have set `ALLAMA__AUTH_TYPES=basic` in your `.env`
...
Allama has a two-level permission system:
...
The first user who logs into Allama is automatically assigned both `superadmin` and an organization admin role.
...
Define the `ALLAMA__AUTH_ALLOWED_DOMAINS` environment variable to restrict the email domains that can log into Allama.
For example, to restrict access to email addresses from `allama.com` and `example.com`, set the following:

```bash
ALLAMA__AUTH_ALLOWED_DOMAINS=allama.com,example.com
```

## Login as admin

<Steps>
  <Step title="Access Allama">
    Go to the Allama UI at `http://localhost` and click the `Sign up` button.
...
To add new users to Allama, the user must first sign-up to Allama.
They will be redirected to the following page:

![new-user](/img/quickstart/admin/new-user.png)

To invite the new user to a workspace, first log into Allama as an organization or workspace admin, then follow these steps:
...
<Steps>
  ...
  <Step title="Login as new user">
    The new user can now log into Allama using the email address and password
...
You can configure Allama to automatically create a workspace for new users when they sign up by enabling `Create workspace on signup` in the application settings.
