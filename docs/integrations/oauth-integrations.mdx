...
Allama supports two OAuth 2.0 grant types for different authentication scenarios:
...
Before configuring OAuth integrations in Allama, you must first create an OAuth application with your chosen provider (Microsoft, Google, GitHub, etc.).
...
2. **(If needed) Configure redirect URI** - Set the callback URL that Allama will provide during setup
...
<AccordionGroup>
  <Accordion title="Integration shows as expired">
    OAuth tokens have limited lifespans. Allama automatically refreshes expired tokens using refresh tokens.
  ...
  <Accordion title="Provider connection fails">
    ...
    - Redirect URI matches the one shown in Allama
