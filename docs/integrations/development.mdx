...
description: Learn how to develop custom integrations for Allama locally.
icon: code-branch
---
...
- A local Allama Docker Compose [deployment](/self-hosting/deployment-options/docker-compose)
- Python 3.12
...
Allama uses [ruff](https://docs.astral.sh/ruff/) and [pre-commit hooks](https://pre-commit.com/) for linting and formatting code.
...
<Steps>
  <CreateNewRepoStep />
  <Step title="Install package and allama_registry">
    ...
    The dev dependencies include the `allama` and `allama_registry[cli]` Python packages.

    Find out more about the `allama_registry` package below.
  </Step>
  <Step title="Setup development environment">
    The final step is to link the local registry to your local Allama instance.
...
<Note>
  `allama_registry` requires `allama` to be installed.
</Note>

Installing `allama_registry` will give you access to the `tc` CLI, which is used to validate your templates before syncing and running them in Allama.
It also includes the `allama_registry` package, which contains necessary imports for building Allama Python UDFs.

### Validation CLI

<Tip>
  We **highly recommend** using this CLI to validate your templates before syncing and running them in Allama.
...
`registry.register`, `Annotated`, `Doc`, and `RegistrySecret` are used to convert your Python function arguments into Allama action inputs.
...
<Note>
  This tutorial assumes you have a local Allama Docker Compose [deployment](/self-hosting/deployment-options/docker-compose).
</Note>

<Warning>
  This is for local development only.

  For production, please sync your custom integrations into Allama from a secure remote repository (e.g. GitHub, GitLab).
  Check out the [custom integrations tutorial](/tutorials/custom-integrations) for more information.
</Warning>

You can point Allama to your local registry by setting the following environment variables in your `.env` file:

```bash
ALLAMA__LOCAL_REPOSITORY_ENABLED=true
ALLAMA__LOCAL_REPOSITORY_PATH=<path-to-local-registry>
# E.g. ALLAMA__LOCAL_REPOSITORY_PATH=~/repos/org/custom-integrations-starter-kit
```

Don't forget to restart Allama:

```bash
docker compose up -d
```

Action templates and Python UDFs will be hot loaded into your Allama instance.
Changes in your local registry are automatically reflected in Allama.
No sync is required.
