---
title: SAML SSO
description: Learn how to authenticate into Allama with SAML SSO.
icon: user-lock
---
...
<Note>
   Allama requires BOTH assertions and responses to be signed.
</Note>
...
<Steps>
    ...
    <Step title="Configure SAML settings">
        - Set the **Single sign-on URL**, **Recipient URL**, and **Destination URL** to `https://<your-allama-instance>/auth/saml/acs`.
        - Set the **Audience Restriction** to `https://<your-allama-instance>`.
    ...
    <Step title="Restart Allama instance">
        Restart Allama to apply the changes.
    </Step>
    <Step title="Test SSO configuration">
        Navigate to your Allama instance and click on the **Login** button.
...
<Steps>
    ...
    <Step title="Configure SAML settings">
        - Set the **Reply URL** and **Sign on URL** to `https://<your-allama-instance>/auth/saml/acs`.
        - Set **Identifier** to `https://<your-allama-instance>/api`.
        - Set **Relay State** to `https://<your-allama-instance>`.
...
<Steps>
    ...
    <Step title="Configure the provider">
        ...
        - Set the **ACS URL** to `https://<your-allama-instance>/auth/saml/acs`.
        - Set the **Audience** to `https://<your-allama-instance>/api`.
...
<Steps>
    <Step title="Create a client">
        ...
        - Set the **Client ID** to `https://<your-allama-instance>/api` and select the **Next** button.

        ![Keycloak Client General Settings](/img/self-hosting/keycloak-client-general.png)
    </Step>
    <Step title="Configure a client">
        - Set the **Valid redirect URIs** to `https://<your-allama-instance>/auth/saml/acs`.
        - Set the **Master SAML Processing URL** to `https://<your-allama-instance>/auth/saml/acs` and select the **Save** button.
    ...
    <Step title="Configure property mapping">
        - Open **Client scopes** tab and click on client scope `https://<your-allama-instance>/api-dedicated`.
...
If you know what you're doing, you can configure the SAML configuration via environment variables in the Allama `api` service:
