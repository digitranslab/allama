...
**Action Templates** are custom integrations built on Allama's YAML-based DSL (domain specific language).
...
<Note>
  Only data specified in the `returns` field of the template is logged in Allama workflows.
...
Templates support all actions defined in the **Allama Registry**.
...
This means that templates have full support for Allama's powerful [inline functions](/quickstart/expressions).
For example:
