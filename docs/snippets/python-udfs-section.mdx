...
Allama makes it easy to turn your Python scripts into no-code workflow actions, which we call **user-defined functions (UDFs)**.
All you need is a:
...
<Note>
  ...
  They are retrieved from Allama's secrets manager at runtime and garbage collected after the action is executed.
</Note>
...
# Register the function as a Allama UDF
@registry.register(
