...
from allama.expressions.common import ExprContext, ExprOperand, ExprType
from allama.expressions.parser.core import parser
from allama.expressions.parser.evaluator import ExprEvaluator
from allama.expressions.validator.validator import BaseExprValidator
from allama.logger import logger
from allama.parse import traverse_expressions
...
class Expression:
    ...
    def visit(self) -> Any:
        match self._visitor:
            ...
            case None:
                raise AllamaExpressionError(
                    "Visitor is required to evaluate an expression"
                )
            case _:
                raise AllamaExpressionError(f"Unsupported visitor: {self._visitor}")
