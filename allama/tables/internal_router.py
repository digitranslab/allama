...
from allama.expressions.functions import tabulate
from allama.logger import logger
from allama.tables.common import coerce_optional_to_utc_datetime
...
async def lookup_rows(
    *,
    ...
    if (
        ...
        and params.limit > config.ALLAMA__MAX_ROWS_CLIENT_POSTGRES
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"Limit cannot be greater than {config.ALLAMA__MAX_ROWS_CLIENT_POSTGRES}"
            ),
        )
    service = TablesService(session, role=role)
    try:
...
async def search_rows(
    *,
    ...
    if params.limit > config.ALLAMA__MAX_ROWS_CLIENT_POSTGRES:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=(
                f"Limit cannot be greater than {config.ALLAMA__MAX_ROWS_CLIENT_POSTGRES}"
