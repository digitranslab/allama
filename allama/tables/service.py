...
from allama.authz.controls import require_workspace_role
from allama.authz.enums import WorkspaceRole
from allama.db.models import Table, TableColumn
from allama.exceptions import (
    AllamaImportError,
    AllamaNotFoundError,
)
from allama.identifiers import TableColumnID, TableID
from allama.identifiers.workflow import WorkspaceUUID
...
from allama.tables.common import (
...
from allama.tables.importer import (
    CSVSchemaInferer,
    InferredCSVColumn,
    generate_table_name,
)
from allama.tables.schemas import (
    TableColumnCreate,
    TableColumnUpdate,
    TableCreate,
    TableRowInsert,
...
class BaseTablesService(BaseWorkspaceService):
    ...
    async def get_table(self, table_id: TableID) -> Table:
        ...
        if table is None:
            raise AllamaNotFoundError("Table not found")
    ...
    async def get_table_by_name(self, table_name: str) -> Table:
        ...
        Raises:
            AllamaNotFoundError: If the table does not exist
        """
        ...
        table = result.scalars().first()
        if table is None:
            raise AllamaNotFoundError(f"Table '{table_name}' not found")
        return table
    ...
    async def create_table(self, params: TableCreate) -> Table:
        ...
        Raises:
            AllamaAuthorizationError: If user lacks required permissions
    ...
    async def get_column(
        ...
        if column is None:
            raise AllamaNotFoundError("Column not found")
        return column
    ...
    async def get_row(self, table: Table, row_id: UUID) -> Any:
        ...
        if row is None:
            raise AllamaNotFoundError(f"Row {row_id} not found in table {table.name}")
        return row
    ...
    async def update_row(
        ...
        Raises:
            AllamaNotFoundError: If the row does not exist
        """
        schema_name = self._get_schema_name()
        conn = await self.session.connection()
    ...
    async def import_table_from_csv(
        ...
        *,
        ...
        if not inferred_columns:
            raise AllamaImportError("CSV file does not contain any columns")
