...
from allama.authz.controls import require_access_level, require_workspace_role
from allama.authz.enums import OrgRole, OwnerType, WorkspaceRole
from allama.cases.service import CaseFieldsService
...
from allama.exceptions import (
    AllamaException,
    AllamaManagementError,
    AllamaNotFoundError,
    AllamaValidationError,
)
from allama.identifiers import InvitationID, UserID, WorkspaceID
...
from allama.workspaces.schemas import (
...
class WorkspaceService(BaseOrgService):
    ...
    async def admin_list_workspaces(
        ...
        if limit is not None:
            if limit <= 0:
                raise AllamaException("List workspace limit must be greater than 0")
    ...
    async def create_invitation(
        ...
        Raises:
            AllamaValidationError: If there is already a pending invitation
    ...
    async def accept_invitation(
        ...
        Raises:
            AllamaNotFoundError: If the invitation is not found.
            AllamaValidationError: If the invitation is expired, already
                accepted, or revoked.
        """
        ...
        if invitation.expires_at < datetime.now(UTC):
            raise AllamaValidationError("Invitation has expired")
        ...
        if result.scalar_one_or_none():
            raise AllamaValidationError("User is already a member of this workspace")
        ...
        if update_result.rowcount == 0:  # pyright: ignore[reportAttributeAccessIssue]
            ...
            if invitation.status == InvitationStatus.ACCEPTED:
                raise AllamaValidationError("Invitation has already been accepted")
            if invitation.status == InvitationStatus.REVOKED:
                raise AllamaValidationError("Invitation has been revoked")
            # Shouldn't reach here, but handle gracefully
            raise AllamaValidationError("Invitation is no longer valid")
    ...
    async def revoke_invitation(
        ...
        Raises:
            AllamaNotFoundError: If the invitation is not found.
            AllamaValidationError: If the invitation is not pending.
        """
