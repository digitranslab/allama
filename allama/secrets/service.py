...
from allama.db.models import BaseSecret, OrganizationSecret, Secret
from allama.exceptions import (
    AllamaAuthorizationError,
    AllamaCredentialsNotFoundError,
    AllamaNotFoundError,
)
from allama.identifiers import SecretID, WorkspaceID
from allama.logger import logger
from allama.registry.constants import REGISTRY_GIT_SSH_KEY_SECRET_NAME
...
from allama.secrets.schemas import (
    SecretCreate,
    SecretKeyValue,
    SecretSearch,
    SecretUpdate,
...
class SecretsService(BaseOrgService):
    ...
    def __init__(self, session: AsyncSession, role: Role | None = None):
        ...
        if not encryption_key:
            raise KeyError("ALLAMA__DB_ENCRYPTION_KEY is not set")
    ...
    async def get_secret_by_name(
        ...
        Raises:
            AllamaNotFoundError: If no secret is found with the given name/environment or if multiple secrets are found
        """
        workspace_id = self._require_workspace_id()
        statement = select(Secret).where(
            Secret.workspace_id == workspace_id,
    ...
    async def get_registry_ssh_key(
        ...
        try:
            ...
            raise AllamaCredentialsNotFoundError(
