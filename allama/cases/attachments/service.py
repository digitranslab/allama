...
from allama.cases.attachments.schemas import CaseAttachmentCreate
from allama.cases.schemas import AttachmentCreatedEvent, AttachmentDeletedEvent
from allama.contexts import ctx_run
from allama.db.models import Case, CaseAttachment, File, Workspace
...
from allama.storage.exceptions import (
    FileSizeError,
    MaxAttachmentsExceededError,
    StorageLimitExceededError,
)
from allama.storage.validation import FileSecurityValidator
...
class CaseAttachmentService(BaseWorkspaceService):
    ...
    def attachments_bucket(self) -> str:
        ...
        return config.ALLAMA__BLOB_STORAGE_BUCKET_ATTACHMENTS
    ...
    async def _upload_to_attachments_bucket(
        ...
        try:
            ...
            raise AllamaException(f"Failed to upload file: {str(e)}") from e
    ...
    async def _emit_case_event(
        ...
        from allama.cases.service import CaseEventsService
    ...
    async def _get_workspace(self) -> Workspace:
        ...
        if self._workspace_cache is None:
            ...
            if not workspace:
                raise AllamaException(f"Workspace {self.workspace_id} not found")
    ...
    async def _assert_case_limits(self, case: Case, new_size: int) -> None:
        ...
        if current_attachment_count >= config.ALLAMA__MAX_ATTACHMENTS_PER_CASE:
            raise MaxAttachmentsExceededError(
                f"Case already has {current_attachment_count} attachments. "
                f"Maximum allowed is {config.ALLAMA__MAX_ATTACHMENTS_PER_CASE}",
                current_count=current_attachment_count,
                max_count=config.ALLAMA__MAX_ATTACHMENTS_PER_CASE,
            )

        current_storage = await self.get_total_storage_used(case)
        if current_storage + new_size > config.ALLAMA__MAX_CASE_STORAGE_BYTES:
            current_mb = current_storage / 1024 / 1024
            new_mb = new_size / 1024 / 1024
            max_mb = config.ALLAMA__MAX_CASE_STORAGE_BYTES / 1024 / 1024
            raise StorageLimitExceededError(
                ...
                max_size=config.ALLAMA__MAX_CASE_STORAGE_BYTES,
            )
    ...
    def _verify_integrity(content: bytes, expected_sha256: str) -> None:
        ...
        if computed_hash != expected_sha256:
            raise AllamaException("File integrity check failed")
    ...
    async def _require_attachment(
        ...
        if not attachment:
            raise AllamaNotFoundError(f"Attachment {attachment_id} not found")
        return attachment
    ...
    async def create_attachment(
        ...
        Raises:
            ...
            AllamaException: If storage operation fails
        """
        ...
        if actual_size > config.ALLAMA__MAX_ATTACHMENT_SIZE_BYTES:
            raise FileSizeError(
                f"File size ({actual_size / 1024 / 1024:.1f}MB) exceeds maximum allowed size "
                f"({config.ALLAMA__MAX_ATTACHMENT_SIZE_BYTES / 1024 / 1024}MB)"
            )
    ...
    async def download_attachment(
        ...
        Raises:
            AllamaNotFoundError: If attachment not found
            AllamaException: If download fails
        """
        ...
        try:
            ...
            raise AllamaNotFoundError("Attachment file not found in storage") from e
        except Exception as e:
            raise AllamaException(f"Failed to download attachment: {str(e)}") from e
    ...
    async def get_attachment_download_url(
        ...
        Raises:
            AllamaNotFoundError: If attachment not found
            AllamaException: If URL generation fails
        """
        ...
        try:
            ...
            raise AllamaException(f"Failed to generate download URL: {str(e)}") from e
    ...
    async def delete_attachment(self, case: Case, attachment_id: uuid.UUID) -> None:
        ...
        Raises:
            AllamaNotFoundError: If attachment not found
            AllamaAuthorizationError: If user lacks permission
        """
        ...
        if (
            ...
            raise AllamaAuthorizationError(
