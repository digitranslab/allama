...
from allama.audit.logger import audit_log
from allama.auth.schemas import UserRead
from allama.auth.types import Role
from allama.cases.attachments import CaseAttachmentService
from allama.cases.durations.service import CaseDurationService
from allama.cases.enums import (
    CaseEventType,
    CasePriority,
    CaseSeverity,
    CaseStatus,
...
from allama.cases.schemas import (
...
from allama.cases.tags.schemas import CaseTagRead
from allama.cases.tags.service import CaseTagsService
from allama.contexts import ctx_run
from allama.custom_fields import CustomFieldsService
from allama.custom_fields.schemas import CustomFieldCreate, CustomFieldUpdate
...
from allama.expressions.expectations import (
    ExpectedField,
    create_expectation_model,
)
from allama.identifiers.workflow import WorkflowUUID
from allama.pagination import (
    BaseCursorPaginator,
    CursorPaginatedResponse,
    CursorPaginationParams,
)
from allama.service import BaseWorkspaceService
from allama.tables.common import normalize_column_options
...
class CasesService(BaseWorkspaceService):
    ...
    async def get_case(
        ...
        Raises:
            AllamaNotFoundError: If the case doesn't exist
        """
        statement = (
            select(Case)
            .where(
    ...
    async def update_case(self, case: Case, params: CaseUpdate) -> Case:
        ...
        Raises:
            AllamaNotFoundError: If the case has no fields when trying to update fields
        """
...
class CaseFieldsService(CustomFieldsService):
    ...
    async def ensure_workspace_row(self, case_id: uuid.UUID) -> uuid.UUID:
        ...
        Raises:
            AllamaException: If the row could not be created or retrieved
        """
        await self._ensure_schema_ready()
    ...
    async def upsert_field_values(
        ...
        Raises:
            AllamaException: If the case has no workspace or if field operations fail
            AllamaNotFoundError: If the row is not found after ensuring it exists
        """
        ...
        except AllamaNotFoundError as e:
            self.logger.error(
...
class CaseCommentsService(BaseWorkspaceService):
    ...
    async def update_comment(
        ...
        Raises:
            AllamaNotFoundError: If the comment doesn't exist
            AllamaAuthorizationError: If the user doesn't own the comment
        """
        # Check if the user owns the comment
        if comment.user_id != self.role.user_id:
            raise AllamaAuthorizationError("You cannot update this comment")
    ...
    async def delete_comment(self, comment: CaseComment) -> None:
        ...
        # Check if the user owns the comment
        if comment.user_id != self.role.user_id:
            raise AllamaAuthorizationError("You can only delete your own comments")
...
class CaseTasksService(BaseWorkspaceService):
    ...
    async def get_task(self, task_id: uuid.UUID) -> CaseTask:
        ...
        Raises:
            AllamaNotFoundError: If the task is not found
        """
        ...
        if not task:
            raise AllamaNotFoundError(f"Task {task_id} not found")
        return task
    ...
    async def _validate_default_trigger_values(
        ...
        Raises:
            AllamaNotFoundError: If the workflow is not found
            AllamaValidationError: If values don't match schema
        """

        if not workflow_id:
            raise AllamaValidationError(
        ...
        if not workflow:
            raise AllamaNotFoundError(f"Workflow {workflow_id} not found")
    ...
    async def create_task(self, case_id: uuid.UUID, params: CaseTaskCreate) -> CaseTask:
        ...
        Raises:
            AllamaNotFoundError: If the case is not found in the current workspace
        """
        ...
        case = result.scalars().first()
        if not case:
            raise AllamaNotFoundError(f"Case {case_id} not found")
    ...
    async def update_task(self, task_id: uuid.UUID, params: CaseTaskUpdate) -> CaseTask:
        ...
        case = result.scalars().first()
        if not case:
            raise AllamaNotFoundError(f"Case {task.case_id} not found")
