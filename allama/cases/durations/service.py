...
from allama.db.models import Case, CaseDuration, CaseEvent
from allama.db.models import CaseDurationDefinition as CaseDurationDefinitionDB
from allama.exceptions import (
    AllamaNotFoundError,
    AllamaValidationError,
)
from allama.service import BaseWorkspaceService
from allama.tables.common import coerce_to_utc_datetime
...
class CaseDurationDefinitionService(BaseWorkspaceService):
    ...
    async def _ensure_unique_name(
        ...
        if result.scalars().first() is not None:
            raise AllamaValidationError(f"A duration named '{name}' already exists")
...
class CaseDurationService(BaseWorkspaceService):
    ...
    async def _resolve_case(self, case: Case | uuid.UUID) -> Case:
        ...
        if resolved is None:
            raise AllamaNotFoundError(f"Case {case} not found in this workspace")
        return resolved
