...
from allama.audit.enums import AuditEventStatus
from allama.audit.logger import audit_log
from allama.audit.service import AuditService
from allama.auth.schemas import SessionRead, UserUpdate
...
from allama.authz.controls import require_access_level
from allama.authz.enums import OrgRole
...
from allama.identifiers import OrganizationID, SessionID, UserID
from allama.invitations.enums import InvitationStatus
from allama.service import BaseOrgService
...
async def accept_invitation_for_user(
    ...
    *,
    ...
    Raises:
        AllamaNotFoundError: If the invitation doesn't exist.
        AllamaAuthorizationError: If the invitation is expired, revoked,
    ...
    if invitation is None:
        raise AllamaNotFoundError("Invitation not found")
    ...
    if user is None:
        raise AllamaAuthorizationError("User not found")
    ...
    if invitation.expires_at < datetime.now(UTC):
        raise AllamaAuthorizationError("Invitation has expired")
    ...
    try:
        ...
        if update_result.rowcount == 0:  # pyright: ignore[reportAttributeAccessIssue]
            ...
            if invitation.status == InvitationStatus.ACCEPTED:
                raise AllamaAuthorizationError("Invitation has already been accepted")
            if invitation.status == InvitationStatus.REVOKED:
                raise AllamaAuthorizationError("Invitation has been revoked")
            # Shouldn't reach here, but handle gracefully
            raise AllamaAuthorizationError("Invitation is no longer valid")
    ...
    except AllamaAuthorizationError:
...
class OrgService(BaseOrgService):
    ...
    async def delete_member(self, user_id: UserID) -> None:
        ...
        Raises:
            AllamaAuthorizationError: If the user is a superuser and cannot be deleted.
        """
        user, _ = await self.get_member(user_id)
        if user.is_superuser:
            raise AllamaAuthorizationError("Cannot delete superuser")
    ...
    async def update_member(
        ...
        Raises:
            AllamaAuthorizationError: If the user is a superuser and cannot be updated.
        """
        user, org_role = await self.get_member(user_id)
        if user.is_superuser:
            raise AllamaAuthorizationError("Cannot update superuser")
        async with self._manager() as user_manager:
    ...
    async def get_invitation_by_token(self, token: str) -> OrganizationInvitation:
        ...
        Raises:
            AllamaNotFoundError: If no invitation with the token exists.
        """
        statement = select(OrganizationInvitation).where(
    ...
    async def revoke_invitation(
        ...
        Raises:
            NoResultFound: If the invitation doesn't exist or belongs to another org.
            AllamaAuthorizationError: If the invitation is not in pending status.
        """
