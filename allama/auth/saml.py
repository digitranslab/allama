...
from allama.db.models import SAMLRequestData
...
async def create_saml_client() -> Saml2Client:
    ...
    saml_settings = {
        "strict": True,
        "entityid": ALLAMA__PUBLIC_API_URL,
        ...
        "service": {
            "sp": {
                "name": "allama_saml_sp",
                "description": "Allama SAML SSO Service Provider",
                "endpoints": {
...
async def sso_acs(
    ...
    *,
    ...
    if role.service_id != "allama-ui":
        logger.warning(
            f"SAML ACS accessed by unexpected service: '{role.service_id}'. "
            f"Expected 'allama-ui' for enhanced security."
        )
        raise HTTPException(
