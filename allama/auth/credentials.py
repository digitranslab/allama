...
from allama.auth.executor_tokens import verify_executor_token
from allama.auth.schemas import UserRole
from allama.auth.types import AccessLevel, PlatformRole, Role
...
from allama.authz.service import MembershipService, MembershipWithOrg
...
from allama.db.models import Organization, OrganizationMembership, User, Workspace
from allama.identifiers import InternalServiceID
from allama.logger import logger

oauth2_scheme = OAuth2PasswordBearer(tokenUrl="token", auto_error=False)
api_key_header_scheme = APIKeyHeader(name="x-allama-service-key", auto_error=False)
...
ORG_OVERRIDE_COOKIE = "allama-org-id"
...
def get_role_from_user(
    ...
    service_id: InternalServiceID = "allama-api",
) -> Role:
...
async def _authenticate_service(
    ...
    service_role_id = request.headers.get("x-allama-role-service-id")
    if service_role_id is None:
        msg = "Missing x-allama-role-service-id header"
        logger.error(msg)
        raise HTTP_EXC(msg)
    if service_role_id not in config.ALLAMA__SERVICE_ROLES_WHITELIST:
        msg = f"x-allama-role-service-id {service_role_id!r} invalid or not allowed"
        logger.error(msg)
        raise HTTP_EXC(msg)
    expected_key = config.ALLAMA__SERVICE_KEY
    if not expected_key:
        raise KeyError("ALLAMA__SERVICE_KEY is not set")
    if not secrets.compare_digest(api_key, expected_key):
        logger.error("Could not validate service key")
        raise CREDENTIALS_EXCEPTION
    user_id = (
        uuid.UUID(uid)
        if (uid := request.headers.get("x-allama-role-user-id")) is not None
        else None
    )
    workspace_id = (
        uuid.UUID(ws_id)
        if (ws_id := request.headers.get("x-allama-role-workspace-id")) is not None
        else None
    )
    workspace_role = (
        WorkspaceRole(ws_role)
        if (ws_role := request.headers.get("x-allama-role-workspace-role"))
        is not None
        else None
    )
    org_role = (
        OrgRole(org_role_str)
        if (org_role_str := request.headers.get("x-allama-role-org-role")) is not None
        else None
    )
    ...
    return Role(
        type="service",
        service_id=service_id,
        access_level=AccessLevel[request.headers["x-allama-role-access-level"]],
        user_id=user_id,
        workspace_id=workspace_id,
        workspace_role=workspace_role,
        org_role=org_role,
...
def TemporaryRole(
    ...
    service_id: InternalServiceID = "allama-service",
):
