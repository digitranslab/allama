...
from allama.config import TEMPORAL__CLUSTER_NAMESPACE
...
def bootstrap_role(organization_id: OrganizationID | None = None) -> Role:
    """Role to bootstrap Allama services.
...
def allama_exception_handler(request: Request, exc: Exception) -> Response:
    """Generic exception handler for Allama exceptions.

    We can customize exceptions to expose only what should be user facing.
    """
    allama_exc = (
        exc if isinstance(exc, AllamaException) else AllamaException(str(exc))
    )
    msg = str(allama_exc)
    logger.error(
        msg,
