...
from allama.db.models import Workflow, WorkflowDefinition, WorkflowFolder
...
from allama.workflow.management.folders.schemas import (
    DirectoryItem,
    FolderDirectoryItem,
    WorkflowDirectoryItem,
)
from allama.workflow.management.schemas import WorkflowDefinitionReadMinimal
...
class WorkflowFolderService(BaseWorkspaceService):
    ...
    async def create_folder(
        ...
        if "/" in name:
            raise AllamaValidationError("Folder name cannot contain slashes")
        ...
        if parent_path != "/":
            ...
            if not parent_exists:
                raise AllamaValidationError(f"Parent path {parent_path} not found")
        ...
        if path_exists:
            raise AllamaValidationError(f"Folder {full_path} already exists")
    ...
    async def rename_folder(
        ...
        if not folder:
            raise AllamaValidationError(f"Folder {folder_id} not found")
        ...
        if new_path != old_path:
            ...
            if not path_exists:
                ...
                pass
            # if path_exists:
            #     raise AllamaValidationError(f"Folder {new_path} already exists")
    ...
    async def move_folder(
        ...
        if new_parent_id is not None:
            ...
            if folder.path == new_parent_path:
                raise AllamaValidationError("Cannot make a folder its own child")

            # Check if new parent is a descendant of the folder (would create a cycle)
            if new_parent.path.startswith(folder.path):
                raise AllamaValidationError("Cannot create cyclic folder structure")
    ...
    async def delete_folder(
        ...
        if folder.path == "/":
            raise AllamaValidationError("Cannot delete root folder")
    ...
    async def get_directory_items(
        ...
        if path != "/":
            ...
            if not folder:
                raise AllamaNotFoundError(f"Folder {path} not found")
            folder_id = folder.id
        else:
            folder_id = None
