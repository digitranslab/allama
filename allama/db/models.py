...
from allama.cases.durations.schemas import CaseDurationAnchorSelection
...
from allama.identifiers import (
    OrganizationID,
    OwnerID,
    WorkspaceID,
    action,
...
from allama.integrations.enums import IntegrationStatus, MCPAuthType, OAuthGrantType
from allama.interactions.enums import InteractionStatus, InteractionType
from allama.invitations.enums import InvitationStatus
from allama.secrets.constants import DEFAULT_SECRETS_ENVIRONMENT
from allama.tiers.types import EntitlementsDict
from allama.workspaces.schemas import WorkspaceSettings
...
class Workflow(WorkspaceModel):
    ...
    registry_lock: Mapped[dict[str, str] | None] = mapped_column(
        ...
        doc=(
            ...
            "Example: {'allama_registry': '1.2.3', 'git+ssh://...': '0.5.0'}"
        ),
    )
...
class Webhook(WorkspaceModel):
    ...
    def secret(self) -> str:
        secret = (
            os.environ.get("ALLAMA__SIGNING_SECRET")
            or config.ALLAMA__SIGNING_SECRET
        )
        if not secret:
            raise ValueError("ALLAMA__SIGNING_SECRET is not set")
    ...
    def url(self) -> str:
        ...
        return f"{config.ALLAMA__PUBLIC_API_URL}/webhooks/{short_wf_id}/{self.secret}"
...
class BaseRegistryVersion(Base):
    ...
    version: Mapped[str] = mapped_column(
        String,
        nullable=False,
        doc="Version string, e.g., '1.0.0', 'allama_core@1.2.3'",
    )
    commit_sha: Mapped[str | None] = mapped_column(
        String,
        nullable=True,
...
class File(WorkspaceModel):
    ...
    name: Mapped[str] = mapped_column(
        String(config.ALLAMA__MAX_ATTACHMENT_FILENAME_LENGTH),
        nullable=False,
