...
def _get_db_uri(driver: Literal["psycopg", "asyncpg"] = "psycopg") -> str:
    ...
    if config.ALLAMA__DB_PASS__ARN:
        logger.info("Retrieving database password from AWS Secrets Manager...")
        try:
            session = boto3.session.Session()
            client = session.client(service_name="secretsmanager")
            response = client.get_secret_value(SecretId=config.ALLAMA__DB_PASS__ARN)
            ...
            username = config.ALLAMA__DB_USER
            password = None
            ...
            if not password:
                if not parsed_json and config.ALLAMA__DB_USER:
            ...
            logger.error(
                ...
                " with `username` and `password` (or set ALLAMA__DB_USER and store the password as the secret string)."
            )
            raise e

        # Get the password from AWS Secrets Manager
        if not config.ALLAMA__DB_ENDPOINT:
            raise ValueError(
                "ALLAMA__DB_ENDPOINT is required when using AWS Secrets Manager"
            )
        if not config.ALLAMA__DB_PORT:
            raise ValueError(
                "ALLAMA__DB_PORT is required when using AWS Secrets Manager"
            )
        if not config.ALLAMA__DB_NAME:
            raise ValueError(
                "ALLAMA__DB_NAME is required when using AWS Secrets Manager"
            )
        uri = get_connection_string(
            username=username,
            password=password,
            host=config.ALLAMA__DB_ENDPOINT,
            port=config.ALLAMA__DB_PORT,
            database=config.ALLAMA__DB_NAME,
            driver=driver,
        )
    ...
    elif config.ALLAMA__DB_USER and config.ALLAMA__DB_PASS:
        if not config.ALLAMA__DB_ENDPOINT:
            raise ValueError(
                "ALLAMA__DB_ENDPOINT is required when using DB credentials"
            )
        if not config.ALLAMA__DB_PORT:
            raise ValueError("ALLAMA__DB_PORT is required when using DB credentials")
        if not config.ALLAMA__DB_NAME:
            raise ValueError("ALLAMA__DB_NAME is required when using DB credentials")
        uri = get_connection_string(
            username=config.ALLAMA__DB_USER,
            password=config.ALLAMA__DB_PASS,
            host=config.ALLAMA__DB_ENDPOINT,
            port=config.ALLAMA__DB_PORT,
            database=config.ALLAMA__DB_NAME,
            driver=driver,
...
def _create_async_db_engine() -> AsyncEngine:
    # Postgres as default
    engine_kwargs = {
        "max_overflow": config.ALLAMA__DB_MAX_OVERFLOW,
        "pool_recycle": config.ALLAMA__DB_POOL_RECYCLE,
        "pool_size": config.ALLAMA__DB_POOL_SIZE,
        "pool_pre_ping": True,
