...
from allama.sandbox.types import SandboxResult
...
class SafePythonExecutor:
    ...
    async def _install_packages(
        ...
        # Add index URLs
        if ALLAMA__SANDBOX_PYPI_INDEX_URL:
            pip_cmd.extend(["--index-url", ALLAMA__SANDBOX_PYPI_INDEX_URL])

        for extra_url in ALLAMA__SANDBOX_PYPI_EXTRA_INDEX_URLS:
    ...
    async def execute(
        ...
        if allow_network:
            logger.warning(
                ...
                "enable nsjail by setting ALLAMA__DISABLE_NSJAIL=false."
            )
