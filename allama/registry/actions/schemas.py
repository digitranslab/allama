...
if TYPE_CHECKING:
    ...
    from allama.db.models import BaseRegistryIndex
    from allama.registry.actions.bound import BoundRegistryAction
    from allama.registry.actions.types import IndexEntry
...
class TemplateActionDefinition(BaseModel):
    ...
    def validate_steps(self):
        ...
        if not self.steps:
            raise AllamaValidationError("Template action must have at least 1 step")
...
class RegistryActionValidateResponse(BaseModel):
    ...
    @staticmethod
    def from_dsl_validation_error(exc: AllamaValidationError):
        return RegistryActionValidateResponse(
