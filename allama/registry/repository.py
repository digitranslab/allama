...
from allama.exceptions import RegistryError
from allama.expressions.expectations import create_expectation_model
from allama.expressions.validation import TemplateValidator
from allama.git.utils import GitUrl, get_git_repository_sha, parse_git_url
from allama.logger import logger
from allama.parse import safe_url
from allama.registry.actions.bound import BoundRegistryAction
from allama.registry.actions.schemas import TemplateAction
from allama.registry.constants import (
    DEFAULT_LOCAL_REGISTRY_ORIGIN,
    DEFAULT_REGISTRY_ORIGIN,
)
from allama.registry.dependencies import (
    RegistryDependencyConflictError,
    get_conflict_summary,
    parse_dependency_conflicts,
)
from allama.registry.fields import (
    Component,
...
from allama.ssh import SshEnv, ssh_context
...
def get_custom_registry_target() -> Path:
    ...
    main allama virtual environment, preventing dependency conflicts.
...
class Repository:
    ...
    def _load_base_udfs(self) -> None:
        ...
        self._register_udfs_from_package(allama_registry)
    ...
    async def load_from_origin(self, commit_sha: str | None = None) -> str | None:
        ...
        # Handle local git repositories
            ...
            if not config.ALLAMA__LOCAL_REPOSITORY_ENABLED:
                raise RegistryError(
                    "Local repository is not enabled on this instance. "
                    "Please set ALLAMA__LOCAL_REPOSITORY_ENABLED=true "
                    "and ensure ALLAMA__LOCAL_REPOSITORY_PATH points to a valid Python package."
                )
            repo_path = Path(config.ALLAMA__LOCAL_REPOSITORY_CONTAINER_PATH)
            ...
            if not repo_path.joinpath("pyproject.toml").exists():
                ...
                if host_path := config.ALLAMA__LOCAL_REPOSITORY_PATH:
                ...
                raise RegistryError(
                    ...
                    "Please ensure ALLAMA__LOCAL_REPOSITORY_PATH points to a valid Python package."
            ...
            # main allama venv, and --python to ensure compatibility with the
            # current interpreter.
            ...
            allowed_domains = cast(
                ...
                await get_setting(
                    ...
                    default=config.ALLAMA__ALLOWED_GIT_DOMAINS,
                )
                or {"github.com"},
            )
    ...
    async def _load_repository(self, repo_url: str, module_name: str) -> ModuleType:
        ...
        # which is separate from the main allama venv.
    ...
    def _register_udf_from_function(
        ...
        *,
        ...
        key = getattr(fn, "__allama_udf_key")
        kwargs = getattr(fn, "__allama_udf_kwargs")
        ...
        *,
        ...
        for name, obj in inspect.getmembers(module):
            ...
            if not inspect.isfunction(obj):
                continue
            is_udf = hasattr(obj, "__allama_udf_key")
            has_udf_kwargs = hasattr(obj, "__allama_udf_kwargs")
...
def generate_model_from_function(
    ...
    for name, param in sig.parameters.items():
        ...
        if metadata := getattr(field_annotation, "__metadata__", None):
            for meta in metadata:
                match meta:
                    ...
                    # `allama_registry` provides lightweight dataclass component
                    # definitions that are not instances of
                    # `allama.registry.fields.Component`. These still need to
                    # propagate to JSONSchema via `x-allama-component` so the frontend
        ...
        if final_components:
            ...
            jsonschema_extra["x-allama-component"] = to_jsonable_python(
                final_components
            )
...
async def install_remote_repository(
    ...
    registry dependencies from the main allama virtual environment.
    This prevents dependency conflicts between allama and user repositories.
    """
