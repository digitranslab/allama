...
(allama.registry.sync) in a subprocess, preventing environment
...
from allama.registry.sync.schemas import (
    SyncResultAdapter,
    SyncResultError,
    SyncResultSuccess,
)
...
async def fetch_actions_from_subprocess(
    ...
    This function invokes `python -m allama.registry.sync` with the given
    parameters, captures stdout (JSON result), and logs stderr.

    Args:
        origin: The repository origin (e.g., "allama_registry", "local", or git URL).
    ...
    cmd = [
        sys.executable,
        "-m",
        "allama.registry.sync.entrypoint",
        "--origin",
        origin,
        "--repository-id",
        str(repository_id),
    ...
    # Inherit current environment and add allama-specific config
    subprocess_env = os.environ.copy()

    # Pass through critical config values as environment variables
    # so the subprocess has access to them
    if config.ALLAMA__LOCAL_REPOSITORY_ENABLED:
        subprocess_env["ALLAMA__LOCAL_REPOSITORY_ENABLED"] = "true"
    if config.ALLAMA__LOCAL_REPOSITORY_PATH:
        subprocess_env["ALLAMA__LOCAL_REPOSITORY_PATH"] = str(
            config.ALLAMA__LOCAL_REPOSITORY_PATH
        )
    if config.ALLAMA__LOCAL_REPOSITORY_CONTAINER_PATH:
        subprocess_env["ALLAMA__LOCAL_REPOSITORY_CONTAINER_PATH"] = str(
            config.ALLAMA__LOCAL_REPOSITORY_CONTAINER_PATH
        )
    if config.ALLAMA__APP_ENV:
        subprocess_env["ALLAMA__APP_ENV"] = config.ALLAMA__APP_ENV
