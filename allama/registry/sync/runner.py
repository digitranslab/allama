...
from allama.registry.sync.platform_service import PLATFORM_REGISTRY_TARBALL_NAMESPACE
from allama.registry.sync.schemas import RegistrySyncRequest, RegistrySyncResult
...
class RegistrySyncRunner:
    ...
    async def run(self, request: RegistrySyncRequest) -> RegistrySyncResult:
        ...
        async with aiofiles.tempfile.TemporaryDirectory(
            ...
            if request.origin_type == "builtin":
                ...
                package_path = Path(config.ALLAMA__LOCAL_REPOSITORY_CONTAINER_PATH)
                commit_sha = None
    ...
    async def _get_builtin_package_path(self) -> Path:
        """Get the path to the builtin allama_registry package.
    ...
    async def _discover_actions(
        ...
        Args:
            ...
            origin: Repository origin (e.g., "allama_registry", "local", or git URL).
