...
def get_builtin_registry_source_path() -> Path:
    """Get the path to the builtin allama_registry package source.

    Checks in order:
    1. Config path (ALLAMA__BUILTIN_REGISTRY_SOURCE_PATH)
    ...
    """
    # Check configured path first (Docker default: /app/packages/allama-registry)
    config_path = Path(config.ALLAMA__BUILTIN_REGISTRY_SOURCE_PATH)
    if (config_path / "pyproject.toml").exists():
        return config_path

    # Fall back to development: look relative to installed package
    package_path = Path(allama_registry.__file__).parent.parent
    ...
    if not pyproject.exists():
        raise TarballBuildError(
            "Cannot find pyproject.toml for allama_registry. "
            "Set ALLAMA__BUILTIN_REGISTRY_SOURCE_PATH or use source installation."
        )
...
async def build_tarball_venv_from_path(
    ...
    if output_dir is None:
        output_dir = Path(tempfile.mkdtemp(prefix="allama_tarball_venv_"))
...
async def build_builtin_registry_tarball_venv(
    ...
    """Build a tarball venv from the builtin allama_registry package.
...
def get_tarball_venv_s3_key(
    ...
    Args:
        ...
        repository_origin: Repository origin (e.g., "allama_registry", "git+ssh://...")
