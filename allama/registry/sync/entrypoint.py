...
Usage:
    python -m allama.registry.sync --origin <origin> [--commit-sha <sha>]
...
from allama.registry.actions.enums import TemplateActionValidationErrorType
from allama.registry.actions.schemas import (
    RegistryActionCreate,
    RegistryActionValidationErrorInfo,
)
from allama.registry.actions.service import validate_action_template
...
from allama.settings.service import get_setting_cached
...
async def load_and_serialize_actions(
    ...
    Args:
        origin: The repository origin (e.g., "allama_registry", "local", or a git URL).
    ...
    # Use "allama-service" as the service ID (valid InternalServiceID)
    # Include organization_id if provided so we can access org-scoped secrets (e.g., SSH keys)
    role = Role(
        type="service", service_id="allama-service", organization_id=organization_id
    )
    ctx_role.set(role)
...
async def main() -> int:
    ...
    parser.add_argument(
        "--origin",
        required=True,
        help="The repository origin (e.g., 'allama_registry', 'local', or a git URL).",
    )
    parser.add_argument(
        "--repository-id",
        required=True,
