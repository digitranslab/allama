...
from allama.dsl.common import context_locator, create_default_execution_context
...
from allama.expressions.common import ExprContext, ExprOperand
...
from allama.parse import traverse_leaves
from allama.registry.actions.bound import BoundRegistryAction
from allama.registry.actions.schemas import TemplateActionDefinition
...
from allama.secrets.common import apply_masks_object
from allama.variables.schemas import VariableSearch
...
async def get_registry_artifacts_for_lock(
    ...
    Args:
        origins: Maps origin -> version string from RegistryLock["origins"].
            Example: {"allama_registry": "2024.12.10.123456", "git+ssh://...": "abc1234"}
...
async def prepare_resolved_context(
    ...
    if config.ALLAMA__UNSAFE_DISABLE_SM_MASKING:
        logger.warning(
...
async def invoke_once(
    ...
    The backend is selected via ALLAMA__EXECUTOR_BACKEND config:
    ...
    timeout = config.ALLAMA__EXECUTOR_CLIENT_TIMEOUT
...
async def dispatch_action(backend: ExecutorBackend, input: RunActionInput) -> Any:
    ...
    Raises:
        AllamaException: If there are errors evaluating for_each expressions or during execution
...
async def get_workspace_variables(
    ...
    *,
    ...
    except AllamaAuthorizationError as e:
