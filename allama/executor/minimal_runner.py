...
It only imports allama_registry (NOT allama) to minimize cold start time.
...
# Only import what we absolutely need - no allama imports!
...
_API_URL = os.environ.get("ALLAMA__API_URL", "http://api:8000")
...
def run_action_minimal(
    ...
    It does NOT import allama - only allama_registry.
    ...
    Args:
        action_impl: Action implementation metadata with:
            ...
            - module: Module path for UDF (e.g., 'allama_registry.integrations.core.transform')
...
async def _run_udf_async(
    ...
    *,
    ...
    try:
        ...
        warnings.warn(
            "Could not import allama_registry._internal.secrets - "
    ...
    try:
        # Import the module from allama_registry
