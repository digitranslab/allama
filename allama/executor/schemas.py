...
from allama.config import ALLAMA__APP_ENV
...
def resolve_backend_type() -> ExecutorBackendType:
    ...
    try:
        backend_type = ExecutorBackendType(config.ALLAMA__EXECUTOR_BACKEND)
    except ValueError:
        valid = ", ".join(f"'{v.value}'" for v in ExecutorBackendType)
        raise ValueError(
            f"Invalid ALLAMA__EXECUTOR_BACKEND: {config.ALLAMA__EXECUTOR_BACKEND!r}. "
...
class ActionImplementation(BaseModel):
    ...
    """Module path for UDF actions (e.g., 'allama_registry.integrations.core.transform')."""
    ...
    """Origin URL for the action's registry (e.g., 'allama_registry' or 'git+ssh://...')."""
...
class ExecutorActionErrorInfo(BaseModel):
    ...
    def __str__(self) -> str:
        ...
        if self.loop_iteration is not None:
            ...
            parts.append(msg)
        if ALLAMA__APP_ENV == "development":
            parts.append(
                f"\n\n{'-' * 30}"
                f"\nFile: {self.filename}"
