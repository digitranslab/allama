...
from allama_registry import secrets as registry_secrets
from allama_registry.context import RegistryContext, set_context
...
from allama.executor.action_runner import get_action_runner
...
from allama.registry.actions.schemas import RegistryActionUDFImpl
from allama.registry.constants import DEFAULT_REGISTRY_ORIGIN
from allama.registry.loaders import load_udf_impl
...
class DirectBackend(ExecutorBackend):
    ...
    def _load_udf_callable(self, action_impl: ActionImplementation):
        ...
        local-reload behavior when ALLAMA__LOCAL_REPOSITORY_ENABLED is set.
        """
    ...
    async def _get_tarball_uris(self, input: RunActionInput, role: Role) -> list[str]:
        ...
        Returns:
            List of tarball URIs in deterministic order (allama_registry first,
    ...
    def _sort_tarball_uris(
        ...
        """Sort tarballs: allama_registry first, then lexicographically by origin."""
