...
from allama.executor.schemas import ExecutorResult, ResolvedContext
...
class WorkerPool:
    ...
    _base_dir: Path = field(
        default_factory=lambda: Path(tempfile.gettempdir()) / "allama-pool-workers"
    )
    ...
    async def start(self) -> None:
        ...
        if config.ALLAMA__EXECUTOR_POOL_METRICS_ENABLED:
    ...
    async def _spawn_worker(self, worker_id: int) -> WorkerInfo:
        ...
        env = os.environ.copy()
        env["ALLAMA_WORKER_ID"] = str(worker_id)
        env["PYTHONDONTWRITEBYTECODE"] = "1"
        env["PYTHONUNBUFFERED"] = "1"

        if config.ALLAMA__DISABLE_NSJAIL:
        ...
        mode = "direct" if config.ALLAMA__DISABLE_NSJAIL else "sandboxed"
        logger.info(
    ...
    async def _spawn_direct_worker(
        ...
        env["ALLAMA_WORKER_SOCKET"] = str(socket_path)

        cmd = [
            sys.executable,
            "-m",
            "allama.executor.backends.pool.worker",
        ]
    ...
    async def _spawn_nsjail_worker(
        ...
        env["ALLAMA_WORKER_SOCKET"] = "/work/task.sock"
        ...
        pythonpath_parts = ["/app", "/app/packages/allama-registry"]
        if site_packages:
            pythonpath_parts.append(site_packages)
        # Add registry cache for custom registry modules distributed via tarball
        registry_cache_dir = Path(config.ALLAMA__EXECUTOR_REGISTRY_CACHE_DIR)
        ...
        nsjail_path = config.ALLAMA__SANDBOX_NSJAIL_PATH
        cmd = [
            nsjail_path,
            "--config",
            str(config_path),
    ...
    def _build_nsjail_config(self, worker_id: int, work_dir: Path) -> str:
        ...
        rootfs = config.ALLAMA__SANDBOX_ROOTFS_PATH
        allama_app = "/app"  # Where allama is installed in container
        ...
        config_lines.extend(
            [
                "",
                "# Allama application",
                f'mount {{ src: "{allama_app}" dst: "/app" is_bind: true rw: false }}',
            ]
        )
        ...
        config_lines.extend(
            [
                ...
                '  arg: "-m"',
                '  arg: "allama.executor.backends.pool.worker"',
                "}",
                "",
                'cwd: "/app"',
            ]
...
async def get_worker_pool(size: int | None = None) -> WorkerPool:
    ...
    2. ALLAMA__EXECUTOR_WORKER_POOL_SIZE env var
    ...
    async with _worker_pool_lock:
        if _worker_pool is None:
            if size is not None:
                pool_size = size
            elif env_size := os.environ.get("ALLAMA__EXECUTOR_WORKER_POOL_SIZE"):
