...
from allama.auth.executor_tokens import mint_executor_token
from allama.executor.schemas import (
    ExecutorActionErrorInfo,
    ResolvedContext,
)
from allama.logger import logger
from allama.sandbox.executor import ActionSandboxConfig, NsjailExecutor
from allama.sandbox.types import ResourceLimits
from allama.storage import blob

if TYPE_CHECKING:
    from allama.auth.types import Role
    from allama.dsl.schemas import RunActionInput

type ExecutionResult = Any | ExecutorActionErrorInfo


def _get_allama_app_dir() -> Path:
    """Get the allama package directory for sandbox mounting.

    Returns the directory containing the allama package, which will be
    mounted read-only in the sandbox at /app.
    """
    # Check if explicitly configured
    if config.ALLAMA__EXECUTOR_ALLAMA_APP_DIR:
        return Path(config.ALLAMA__EXECUTOR_ALLAMA_APP_DIR)

    # Auto-detect from installed package location
    import allama

    allama_path = Path(allama.__file__).parent
    # Return parent (site-packages) so sandbox can import allama
    return allama_path.parent


def _get_site_packages_dir() -> Path | None:
    """Get the site-packages directory containing Python dependencies.

    Returns the site-packages directory where allama's dependencies
    (orjson, pydantic, sqlalchemy, etc.) are installed.
    """
    # Check if explicitly configured
    if config.ALLAMA__EXECUTOR_SITE_PACKAGES_DIR:
        return Path(config.ALLAMA__EXECUTOR_SITE_PACKAGES_DIR)
...
def _is_sandbox_available() -> bool:
    ...
    nsjail_path = Path(config.ALLAMA__SANDBOX_NSJAIL_PATH)
    rootfs_path = Path(config.ALLAMA__SANDBOX_ROOTFS_PATH)
...
class ActionRunner:
    ...
    def __init__(self, cache_dir: Path | None = None):
        self.cache_dir = cache_dir or Path(config.ALLAMA__EXECUTOR_REGISTRY_CACHE_DIR)
    ...
    async def execute_action(
        ...
        """
        timeout = timeout or config.ALLAMA__EXECUTOR_CLIENT_TIMEOUT
        ...
        use_sandbox = force_sandbox or (
            config.ALLAMA__EXECUTOR_SANDBOX_ENABLED and _is_sandbox_available()
        )
        logger.debug(
    ...
    async def _execute_sandboxed(
        ...
        job_dir = Path(tempfile.mkdtemp(prefix="allama_action_"))
        ...
        try:
            ...
            from allama.executor import minimal_runner as minimal_runner_module
            ...
            sandbox_env["ALLAMA__API_URL"] = config.ALLAMA__API_URL
            sandbox_env["ALLAMA__WORKSPACE_ID"] = (
                str(role.workspace_id) if role.workspace_id else ""
            )
            sandbox_env["ALLAMA__WORKFLOW_ID"] = str(input.run_context.wf_id)
            sandbox_env["ALLAMA__RUN_ID"] = str(input.run_context.wf_run_id)
            sandbox_env["ALLAMA__WF_EXEC_ID"] = str(input.run_context.wf_exec_id)
            sandbox_env["ALLAMA__ENVIRONMENT"] = input.run_context.environment
            ...
            sandbox_env["ALLAMA__EXECUTOR_TOKEN"] = executor_token
            ...
            sandbox_config = ActionSandboxConfig(
                registry_paths=registry_paths,
                allama_app_dir=_get_allama_app_dir(),
                site_packages_dir=_get_site_packages_dir(),
                env_vars=sandbox_env,
                resources=ResourceLimits(
                    memory_mb=config.ALLAMA__SANDBOX_DEFAULT_MEMORY_MB,
