...
with workflow.unsafe.imports_passed_through():
    ...
    from allama.agent.schemas import RunAgentArgs
    from allama.agent.session.types import AgentSessionEntity
    from allama.agent.types import AgentConfig
    from allama.concurrency import cooperative
    from allama.contexts import (
        ctx_interaction,
        ctx_logger,
        ctx_logical_time,
        ctx_role,
    ...
    from allama.dsl.scheduler import DSLScheduler
    ...
    from allama.dsl.types import ActionErrorInfo, ActionErrorInfoAdapter
    from allama.dsl.validation import (
        ResolveTimeAnchorActivityInputs,
        format_input_schema_validation_error,
        resolve_time_anchor_activity,
    )
    from allama.ee.interactions.decorators import maybe_interactive
    from allama.ee.interactions.schemas import InteractionInput, InteractionResult
    from allama.ee.interactions.service import InteractionManager
    from allama.exceptions import (
        AllamaException,
        AllamaExpressionError,
        AllamaNotFoundError,
    )
    from allama.expressions.eval import is_template_only
    from allama.identifiers import WorkspaceID
    ...
    from allama.validation.schemas import ValidationDetailListTA
    from allama.workflow.executions.enums import (
        ExecutionType,
        TemporalSearchAttr,
        TriggerType,
    )
    from allama.workflow.executions.types import ErrorHandlerWorkflowInput
    from allama.workflow.management.definitions import (
        get_workflow_definition_activity,
        resolve_registry_lock_activity,
    )
    from allama.workflow.management.management import WorkflowsManagementService
    from allama.workflow.management.schemas import (
    ...
    from allama.workflow.schedules.schemas import GetScheduleActivityInputs
    from allama.workflow.schedules.service import WorkflowSchedulesService
...
class DSLWorkflow:
    ...
    def __init__(self, args: DSLRunArgs) -> None:
        ...
        # Allama wf exec id == Temporal wf exec id
        self.wf_exec_id = wf_info.workflow_id
        # Allama wf run id == Temporal wf run id
    ...
    async def _run_workflow(self, args: DSLRunArgs) -> StoredObject:
        ...
        except AllamaExpressionError as e:
            raise ApplicationError(
            ...
            match task.action:
                ...
                case PlatformAction.AI_AGENT:
                    ...
                    action_result = await workflow.execute_child_workflow(
                        ...
                        task_queue=config.ALLAMA__AGENT_QUEUE,
                        execution_timeout=wf_info.execution_timeout,
                        task_timeout=wf_info.task_timeout,
    ...
    async def _handle_return(self) -> StoredObject:
        ...
        if self.dsl.returns is None:
            match config.ALLAMA__WORKFLOW_RETURN_STRATEGY:
                case "context":
    ...
    async def _get_error_handler_workflow_id(
        ...
        This is done by checking if the error is a AllamaValidationError or
        AllamaExpressionError.
        """
        return await workflow.execute_activity(
        ...
        if match := re.match(identifiers.workflow.WF_EXEC_ID_PATTERN, orig_wf_exec_id):
            if self.role.workspace_id is None:
                ...
                try:
                    ...
                    url = (
                        f"{config.ALLAMA__PUBLIC_APP_URL}/workspaces/{self.role.workspace_id}"
