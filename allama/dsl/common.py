...
from allama.db.models import Action
from allama.dsl._converter import PydanticPayloadConverter
...
from allama.expressions.core import extract_expressions
from allama.expressions.expectations import ExpectedField
from allama.identifiers import ActionID
from allama.identifiers.schedules import ScheduleUUID
from allama.identifiers.workflow import AnyWorkflowID, WorkflowUUID
from allama.interactions.schemas import ActionInteractionValidator
from allama.logger import logger
from allama.registry.lock.types import RegistryLock
from allama.storage.object import CollectionObject, InlineObject, StoredObject
from allama.workflow.actions.schemas import ActionControlFlow
...
class DSLInput(BaseModel):
    ...
    def validate_structure(self) -> Self:
        if not self.actions:
            raise AllamaDSLError("At least one action must be defined.")
        ...
        if n_entrypoints == 0:
            raise AllamaDSLError("No entrypoints found")
    ...
    def _assign_action_scopes(
        ...
        Raises AllamaDSLError if an action is assigned to multiple scopes.
        """
        ...
        if n_visited != len(self.actions):
            raise AllamaDSLError("Cycle detected in scatter-gather workflow")
    ...
    def from_yaml(path: str | Path | SpooledTemporaryFile) -> DSLInput:
        ...
        if isinstance(path, str | Path):
            ...
            raise AllamaDSLError(f"Invalid file/path type {type(path)}")
        dsl_dict = yaml.safe_load(yaml_str)
        try:
            return DSLInput.model_validate(dsl_dict)
        except* AllamaDSLError as eg:
...
NON_RETRYABLE_ERROR_TYPES = [
    ...
    # Allama
    AllamaException.__name__,
    AllamaExpressionError.__name__,
    AllamaValidationError.__name__,
    AllamaDSLError.__name__,
    AllamaCredentialsError.__name__,
    # Temporal
