...
from allama.common import is_iterable
...
from allama.dsl.enums import StreamErrorHandlingStrategy
...
from allama.dsl.validation import normalize_trigger_inputs
...
from allama.storage.collection import (
...
from allama.validation.schemas import ValidationDetail
...
class DSLActivities:
    ...
    def evaluate_single_expression_activity(
        ...
        no longer fail silently – instead we raise a ``AllamaExpressionError``
        ...
        if not expr_str:
            raise AllamaExpressionError("Expression cannot be empty")
        ...
        # Internally, this will raise a ``AllamaExpressionError`` if the expression
    ...
    async def synchronize_collection_object_activity(
        ...
        if config.ALLAMA__RESULT_EXTERNALIZATION_ENABLED:
