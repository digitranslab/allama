...
with workflow.unsafe.imports_passed_through():
    ...
    from allama.dsl.common import DSLRunArgs, get_trigger_type
...
def _set_fingerprint(
    ...
    if len(input.args) > 0 and isinstance(input.args[0], DSLRunArgs):
        ...
        sentry.set_tag("allama.workflow_id", wf_id)

        # Set additional tags for the workflow from its DSLRunArgs
        if parent_run_context := arg.parent_run_context:
            sentry.set_tag("allama.parent.workflow_id", str(parent_run_context.wf_id))
            sentry.set_tag(
                "allama.parent.workflow_exec_id", parent_run_context.wf_exec_id
            )
        if dsl := arg.dsl:
            sentry.set_tag("allama.dsl.title", dsl.title)
            sentry.set_tag("allama.dsl.description", dsl.description)
            sentry.set_tag("allama.dsl.error_handler", dsl.error_handler)
            sentry.set_tag("allama.dsl.config.timeout", dsl.config.timeout)
    else:
        logger.warning(
    ...
    async def execute_workflow(self, input: ExecuteWorkflowInput) -> Any:
        ...
        with sentry.isolation_scope() as scope:
            ...
            if (role := ctx_role.get()) and role.workspace_id:
                sentry.set_tag("allama.workspace_id", str(role.workspace_id))
            sentry.set_tag("allama.trigger_type", trigger_type.value)
            ...
            try:
                ...
                # Get the temporal search attribute for AllamaTriggerType
                match trigger_type:
