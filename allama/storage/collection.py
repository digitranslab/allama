...
Usage:
    from allama.storage.collection import store_collection, materialize_collection_values
...
class CollectionManifestV1(BaseModel):
    ...
    kind: Literal["allama.collection_manifest"] = "allama.collection_manifest"
...
class CollectionChunkV1(BaseModel):
    ...
    kind: Literal["allama.collection_chunk"] = "allama.collection_chunk"
...
async def store_collection(
    ...
    chunk_size = chunk_size or config.ALLAMA__COLLECTION_CHUNK_SIZE
    bucket = bucket or config.ALLAMA__BLOB_STORAGE_BUCKET_WORKFLOW
