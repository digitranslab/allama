...
class FileSecurityValidator:
    ...
    def __init__(
        ...
        self.max_file_size = max_file_size or config.ALLAMA__MAX_ATTACHMENT_SIZE_BYTES
        self.max_filename_length = (
            max_filename_length or config.ALLAMA__MAX_ATTACHMENT_FILENAME_LENGTH
        )
        # Normalize extensions: trim whitespace and lowercase for robust comparison
        raw_exts = list(
            allowed_extensions or config.ALLAMA__ALLOWED_ATTACHMENT_EXTENSIONS
        )
        self.allowed_extensions = [
            e.strip().lower() for e in raw_exts if str(e).strip()
        ]
        self.allowed_mime_types = list(
            allowed_mime_types or config.ALLAMA__ALLOWED_ATTACHMENT_MIME_TYPES
        )
