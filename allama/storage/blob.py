...
async def get_storage_client() -> AsyncIterator[S3Client]:
    ...
    if config.ALLAMA__BLOB_STORAGE_ENDPOINT is not None:
        # MinIO configuration - use AWS_* or MINIO_ROOT_* credentials
        async with session.client(
            "s3",
            endpoint_url=config.ALLAMA__BLOB_STORAGE_ENDPOINT,
...
async def generate_presigned_download_url(
    ...
    """
    expiry = expiry or config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_EXPIRY
    ...
    async with get_storage_client() as s3_client:
        try:
            ...
            if (
                config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_ENDPOINT is not None
                and config.ALLAMA__BLOB_STORAGE_ENDPOINT is not None
            ):
                url = url.replace(
                    config.ALLAMA__BLOB_STORAGE_ENDPOINT,
                    config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_ENDPOINT,
                )
            return url
        except ClientError as e:
            logger.error(
