...
def normalize_mcp_tool_name(mcp_tool_name: str) -> str:
    ...
    Handles Allama registry tools:
    - mcp__allama-registry__tools__slack__post_message -> tools.slack.post_message
    - mcp.allama-registry.core.cases.create_case -> core.cases.create_case

    Handles user MCP servers routed through the proxy:
    - mcp__allama-registry__mcp__Linear__list_issues -> mcp.Linear.list_issues
    - mcp.allama-registry.mcp.Linear.list_issues -> mcp.Linear.list_issues
    ...
    # Pattern: mcp.allama-registry.mcp.{server}.{tool}
    if mcp_tool_name.startswith("mcp.allama-registry.mcp."):
        # Extract mcp.{server}.{tool} part
        return mcp_tool_name.replace("mcp.allama-registry.", "", 1)

    # Handle user MCP tools routed through proxy (underscore-separated, runtime)
    # Pattern: mcp__allama-registry__mcp__{server}__{tool}
    if mcp_tool_name.startswith("mcp__allama-registry__mcp__"):
        # Extract mcp__{server}__{tool} part and convert to mcp.{server}.{tool}
        tool_part = mcp_tool_name.replace("mcp__allama-registry__", "")
        return mcp_tool_name_to_action_name(tool_part)

    # Handle dot-separated format (persisted messages) for registry tools
    if mcp_tool_name.startswith("mcp.allama-registry."):
        return mcp_tool_name.replace("mcp.allama-registry.", "", 1)

    # Handle underscore-separated format (runtime MCP tool names) for registry tools
    if mcp_tool_name.startswith("mcp__allama-registry__"):
        tool_part = mcp_tool_name.replace("mcp__allama-registry__", "")
        return mcp_tool_name_to_action_name(tool_part)
