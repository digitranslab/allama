...
import allama.agent.adapter.vercel
from allama import config
from allama.agent.approvals.enums import ApprovalStatus
from allama.agent.preset.prompts import AgentPresetBuilderPrompt
from allama.agent.preset.service import AgentPresetService
from allama.agent.schemas import RunAgentArgs
from allama.agent.service import AgentManagementService
from allama.agent.session.schemas import (
    AgentSessionCreate,
    AgentSessionRead,
    AgentSessionUpdate,
)
from allama.agent.session.types import AgentSessionEntity
from allama.agent.types import AgentConfig, ClaudeSDKMessageTA
from allama.audit.logger import audit_log
from allama.cases.prompts import CaseCopilotPrompts
from allama.cases.service import CasesService
from allama.chat.enums import MessageKind
from allama.chat.schemas import (
    ApprovalRead,
    BasicChatRequest,
    ChatMessage,
    ChatReadMinimal,
...
from allama.chat.service import ChatService
from allama.chat.tools import get_default_tools
from allama.db.models import AgentSession, AgentSessionHistory, Approval, Chat
from allama.dsl.client import get_temporal_client
from allama.dsl.common import RETRY_POLICIES
from allama.exceptions import AllamaNotFoundError
from allama.identifiers import UserID
from allama.logger import logger
from allama.service import BaseWorkspaceService
from allama.workspaces.prompts import WorkspaceCopilotPrompts

if TYPE_CHECKING:
    from allama.agent.executor.activity import ToolExecutionResult
...
class AgentSessionService(BaseWorkspaceService):
    ...
    async def run_turn(
        ...
        Raises:
            AllamaNotFoundError: If the session is not found.
        ...
        if not agent_session:
            raise AllamaNotFoundError(f"Session with ID {session_id} not found")
        ...
        match request:
            ...
            case VercelChatRequest(message=ui_message):
                [message] = allama.agent.adapter.vercel.convert_ui_message(ui_message)
                match message:
    ...
    async def _continue_with_approvals(
        ...
        Raises:
            AllamaNotFoundError: If no active session exists.
        """
        from allama_ee.agent.approvals.service import ApprovalMap
        from allama_ee.agent.types import AgentWorkflowID
        from allama_ee.agent.workflows.durable import (
            DurableAgentWorkflow,
    ...
    async def _build_agent_config(
        ...
        Raises:
            ...
            AllamaNotFoundError: If required resources are not found.
        """
        ...
        if session_entity is AgentSessionEntity.CASE:
            ...
            except AllamaNotFoundError as exc:
                raise ValueError(
    ...
    async def _entity_to_prompt(self, agent_session: AgentSession) -> str:
        ...
        if entity_type == AgentSessionEntity.CASE:
            ...
            if not case:
                raise AllamaNotFoundError(f"Case with ID {entity_id} not found")
    ...
    async def replace_interrupt_with_tool_results(
        ...
        if not session:
            raise AllamaNotFoundError(f"Session {session_id} not found")
    ...
    async def fork_session(
        ...
        *,
        ...
        Raises:
            AllamaNotFoundError: If the parent session is not found.
        """
