...
- Site-packages mounted read-only for Claude SDK deps and allama package

Key design:
- Runtime executed via `python -m allama.agent.sandbox.entrypoint`
- Mount site-packages read-only for deps (includes allama package)
- Control socket at /var/run/allama/control.sock
- LLM socket at /var/run/allama/llm.sock (proxied to LiteLLM)
"""
...
from allama.agent.common.exceptions import AgentSandboxValidationError

# Well-known socket paths (internal to agent worker, not configurable)
TRUSTED_MCP_SOCKET_PATH = Path("/var/run/allama/mcp.sock")
CONTROL_SOCKET_NAME = "control.sock"
JAILED_CONTROL_SOCKET_PATH = Path("/var/run/allama/control.sock")

# LLM socket for proxied access to LiteLLM (network isolated)
LLM_SOCKET_NAME = "llm.sock"
JAILED_LLM_SOCKET_PATH = Path("/var/run/allama/llm.sock")
...
class AgentResourceLimits:
    ...
    - ALLAMA__AGENT_SANDBOX_MEMORY_MB: memory_mb (default 4096 = 4 GiB)
    - ALLAMA__AGENT_SANDBOX_TIMEOUT: timeout_seconds and cpu_seconds (default 600s)
    ...
    memory_mb: int = field(default_factory=lambda: ALLAMA__AGENT_SANDBOX_MEMORY_MB)
    cpu_seconds: int = field(default_factory=lambda: ALLAMA__AGENT_SANDBOX_TIMEOUT)
    max_file_size_mb: int = 256
    max_open_files: int = 512
    max_processes: int = 128
    timeout_seconds: int = field(
        default_factory=lambda: ALLAMA__AGENT_SANDBOX_TIMEOUT
    )
...
AGENT_SANDBOX_BASE_ENV = {
    ...
    # PYTHONPATH: /app for allama package, /site-packages for dependencies
...
def build_agent_nsjail_config(
    ...
    *,
    ...
    Args:
        ...
        allama_pkg_dir: Path to the allama package directory.
    ...
    _validate_path(allama_pkg_dir, "allama_pkg_dir")
    ...
    lines.extend(
        [
            ...
            "",
            "# Allama package - minimal subdirectories for fast cold start",
            ...
            f'mount {{ src: "{allama_pkg_dir}/__init__.py" dst: "/app/allama/__init__.py" is_bind: true rw: false }}',
            f'mount {{ src: "{allama_pkg_dir}/agent/__init__.py" dst: "/app/allama/agent/__init__.py" is_bind: true rw: false }}',
            "",
            ...
            f'mount {{ src: "{allama_pkg_dir}/logger" dst: "/app/allama/logger" is_bind: true rw: false }}',
            f'mount {{ src: "{allama_pkg_dir}/agent/common" dst: "/app/allama/agent/common" is_bind: true rw: false }}',
            f'mount {{ src: "{allama_pkg_dir}/agent/runtime" dst: "/app/allama/agent/runtime" is_bind: true rw: false }}',
            f'mount {{ src: "{allama_pkg_dir}/agent/sandbox" dst: "/app/allama/agent/sandbox" is_bind: true rw: false }}',
            f'mount {{ src: "{allama_pkg_dir}/agent/mcp" dst: "/app/allama/agent/mcp" is_bind: true rw: false }}',
            "",
            "# Trusted MCP socket (read-only, shared across jobs)",
            f'mount {{ src: "{TRUSTED_MCP_SOCKET_PATH.parent}" dst: "/var/run/allama" is_bind: true rw: false }}',
            "",
    ...
    # Execution settings - run allama.agent.sandbox.entrypoint module
    ...
    lines.extend(
        [
            ...
            'cwd: "/work"',
            'exec_bin { path: "/usr/local/bin/python3" arg: "-m" arg: "allama.agent.sandbox.entrypoint" }',
        ]
    )
