...
Uses the same rootfs as the action sandbox (ALLAMA__SANDBOX_ROOTFS_PATH).
Site-packages are mounted read-only for Claude SDK and allama dependencies.

When ALLAMA__DISABLE_NSJAIL=true, the runtime is spawned as a direct
...
from allama.agent.common.config import ALLAMA__DISABLE_NSJAIL
from allama.agent.common.exceptions import (
    AgentSandboxExecutionError,
    AgentSandboxTimeoutError,
)
from allama.agent.sandbox.config import (
    AgentSandboxConfig,
    build_agent_env_map,
...
def _get_site_packages_dir() -> Path:
    """Get the site-packages directory containing allama dependencies."""
...
def _get_allama_pkg_dir() -> Path:
    """Get the allama package directory.

    This finds the allama package directory itself, which is typically
    /app/allama in Docker or {project_root}/allama in development.
    """
    import allama

    # allama.__file__ is /app/allama/__init__.py, we want /app/allama
    return Path(allama.__file__).parent
...
async def spawn_jailed_runtime(
    ...
    nsjail_path: str = ALLAMA__SANDBOX_NSJAIL_PATH,
    rootfs_path: str = ALLAMA__SANDBOX_ROOTFS_PATH,
    *,
...
async def _spawn_direct_runtime() -> SpawnedRuntime:
    ...
    Python environment. Used when ALLAMA__DISABLE_NSJAIL=true.
    ...
    cmd = [
        sys.executable,
        "-m",
        "allama.agent.sandbox.entrypoint",
    ]
...
async def _spawn_nsjail_runtime(
    ...
    *,
    ...
    The runtime uses allama.agent.sandbox.entrypoint which is available via
    ...
    # Get site-packages and allama package directories
    site_packages_dir = _get_site_packages_dir()
    allama_pkg_dir = _get_allama_pkg_dir()
