...
This module provides harness-agnostic tool building from the Allama registry.
...
from allama.agent.types import Tool
from allama.config import ALLAMA__AGENT_MAX_TOOLS
...
from allama.registry.loaders import get_bound_action_from_manifest
...
async def call_allama_action(
    action_name: str,
    args: dict[str, Any],
) -> Any:
    """Execute a Allama action in a subprocess.
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        proc = await asyncio.create_subprocess_exec(
            sys.executable,
            "-m",
            "allama.agent._subprocess_runner",
            "--input",
            str(input_path),
            "--output",
            str(output_path),
...
async def build_agent_tools(
    ...
    tool_approvals: dict[str, bool] | None = None,
    max_tools: int = ALLAMA__AGENT_MAX_TOOLS,
) -> BuildToolsResult:
