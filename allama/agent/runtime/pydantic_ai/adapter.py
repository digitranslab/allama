...
Also provides conversion functions between Allama's harness-agnostic
...
from allama.agent.tools import call_allama_action
from allama.agent.types import (
...
def from_pydantic_ai_deferred_requests(
    ...
    """Convert pydantic-ai DeferredToolRequests to Allama's harness-agnostic type."""
...
def to_pydantic_ai_deferred_results(
    ...
    """Convert Allama's DeferredToolResults to pydantic-ai's type for agent.run()."""
...
def to_pydantic_ai_tool(tool: Tool) -> PATool[Any]:
    """Convert a Allama Tool to a pydantic-ai Tool.
    ...
    Args:
        tool: Allama's harness-agnostic Tool with canonical name
    ...
    async def tool_func(**kwargs: Any) -> Any:
        return await call_allama_action(action_name, kwargs)
...
def to_pydantic_ai_tools(tools: list[Tool]) -> list[PATool]:
    """Convert a list of Allama Tools to pydantic-ai Tools.

    Args:
        tools: List of Allama's harness-agnostic Tools
