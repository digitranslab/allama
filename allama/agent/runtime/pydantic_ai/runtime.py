...
from allama.agent.exceptions import AgentRunError
from allama.agent.executor.aio import AioStreamingAgentExecutor
from allama.agent.parsers import try_parse_json
from allama.agent.schemas import AgentOutput, RunAgentArgs, RunUsage
from allama.agent.stream.common import PersistableStreamingAgentDeps
from allama.agent.types import AgentConfig, MCPServerConfig, OutputType
from allama.chat.schemas import ChatMessage
from allama.config import (
    ALLAMA__AGENT_MAX_REQUESTS,
    ALLAMA__AGENT_MAX_RETRIES,
    ALLAMA__AGENT_MAX_TOOL_CALLS,
)
from allama.contexts import ctx_role, ctx_session_id
from allama.exceptions import AllamaAuthorizationError
...
async def run_agent_sync(
    ...
    *,
    ...
    if max_tools_calls and max_tools_calls > ALLAMA__AGENT_MAX_TOOL_CALLS:
        raise ValueError(
            f"Cannot request more than {ALLAMA__AGENT_MAX_TOOL_CALLS} tool calls"
        )
    if max_requests > ALLAMA__AGENT_MAX_REQUESTS:
        raise ValueError(
            f"Cannot request more than {ALLAMA__AGENT_MAX_REQUESTS} requests"
        )
...
async def run_agent(
    ...
    max_tool_calls: int = ALLAMA__AGENT_MAX_TOOL_CALLS,
    max_requests: int = ALLAMA__AGENT_MAX_REQUESTS,
    retries: int = ALLAMA__AGENT_MAX_RETRIES,
    base_url: str | None = None,
    deferred_tool_results: DeferredToolResults | None = None,
) -> AgentOutput:
    """Run an AI agent with specified configuration and actions.

    This function creates and executes a Allama AI agent with the provided
    ...
    if max_tool_calls > ALLAMA__AGENT_MAX_TOOL_CALLS:
        raise ValueError(
            f"Cannot request more than {ALLAMA__AGENT_MAX_TOOL_CALLS} tool calls"
        )
    ...
    if role is None or role.workspace_id is None:
        raise AllamaAuthorizationError("Workspace context required for agent run")
