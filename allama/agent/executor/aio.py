...
from allama.agent.executor.base import BaseAgentExecutor, BaseAgentRunHandle
from allama.agent.factory import AgentFactory, build_agent
from allama.agent.schemas import RunAgentArgs
from allama.agent.stream.events import StreamError
from allama.agent.stream.writers import event_stream_handler
from allama.agent.types import StreamingAgentDeps
from allama.auth.types import Role
from allama.chat.constants import APPROVAL_REQUEST_HEADER
from allama.config import ALLAMA__UNIFIED_AGENT_STREAMING_ENABLED
...
class AioStreamingAgentExecutor(BaseAgentExecutor[ExecutorResult]):
    ...
    async def _start_agent(self, args: RunAgentArgs) -> ExecutorResult:
        ...
        if not args.is_continuation:
            ...
            if ALLAMA__UNIFIED_AGENT_STREAMING_ENABLED:
