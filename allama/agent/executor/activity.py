...
from allama.agent.common.exceptions import AgentSandboxExecutionError
from allama.agent.common.stream_types import ToolCallContent, UnifiedStreamEvent
from allama.agent.common.types import MCPToolDefinition
from allama.agent.executor.loopback import (
    LoopbackHandler,
    LoopbackInput,
)
from allama.agent.mcp.executor import ActionExecutionError, execute_action
from allama.agent.mcp.utils import normalize_mcp_tool_name
from allama.agent.sandbox.llm_proxy import LLM_SOCKET_NAME, LLMSocketProxy
from allama.agent.sandbox.nsjail import spawn_jailed_runtime
from allama.agent.session.service import AgentSessionService
from allama.agent.stream.connector import AgentStream
from allama.agent.tokens import MCPTokenClaims
...
class SandboxedAgentExecutor:
        ...
        try:
            ...
            async with server:
                ...
                logger.info(
                    ...
                    session_id=self.input.session_id,
                    mode="direct" if ALLAMA__DISABLE_NSJAIL else "nsjail",
                )
    ...
    async def _create_job_directory(self) -> Path:
        ...
        base_dir = Path("/tmp/allama-agent-jobs")
...
async def run_agent_activity(
    ...
    When ALLAMA__DISABLE_NSJAIL=true, the agent is run as a direct subprocess
    ...
    """
    sandbox_mode = "direct" if ALLAMA__DISABLE_NSJAIL else "nsjail"
    activity.heartbeat(
