...
with workflow.unsafe.imports_passed_through():
    import sentry_sdk
    import uvloop
    from allama_ee.agent.activities import AgentActivities
    from allama_ee.agent.approvals.service import ApprovalManager
    from allama_ee.agent.workflows.durable import DurableAgentWorkflow

    from allama import config
    from allama.agent.executor.activity import (
        execute_approved_tools_activity,
        run_agent_activity,
    )
    from allama.agent.mcp.trusted_server import app
    from allama.agent.preset.activities import (
        resolve_agent_preset_config_activity,
    )
    from allama.agent.session.activities import get_session_activities
    from allama.dsl.client import get_temporal_client
    from allama.dsl.interceptor import SentryInterceptor
...
async def start_litellm_proxy() -> None:
    ...
    # Use the config file from the allama.agent package
    ...
    # Create symlink at /app so that "allama.agent.gateway" resolves correctly
    # (dirname(/app/litellm_config.yaml) + allama/agent/gateway.py = /app/allama/agent/gateway.py)
    ...
    # This allows LiteLLM to import allama modules for custom auth/callbacks
    env = os.environ.copy()
    pythonpath = env.get("PYTHONPATH", "")
    app_paths = "/app:/app/packages/allama-registry:/app/packages/allama-ee"
...
async def start_mcp_server() -> None:
    ...
    socket_path = Path("/var/run/allama/mcp.sock")
...
async def main() -> None:
    """Run the AgentWorker."""
    max_concurrent = int(
        os.environ.get("ALLAMA__AGENT_MAX_CONCURRENT_ACTIVITIES", 100)
    )
    ...
    try:
        client = await get_temporal_client(plugins=[AllamaPydanticAIPlugin()])

        interceptors = []
        if sentry_dsn := os.environ.get("SENTRY_DSN"):
            logger.info("Initializing Sentry interceptor")
            app_env = config.ALLAMA__APP_ENV
