...
from allama.agent.config import MODEL_CONFIGS, PROVIDER_CREDENTIAL_CONFIGS
from allama.agent.preset.service import AgentPresetService
from allama.agent.schemas import (
    ModelConfig,
...
from allama.db.models import OrganizationSecret
from allama.exceptions import AllamaAuthorizationError, AllamaNotFoundError
from allama.logger import logger
from allama.secrets import secrets_manager
from allama.secrets.enums import SecretType
from allama.secrets.schemas import SecretCreate, SecretKeyValue, SecretUpdate
from allama.secrets.service import SecretsService
from allama.service import BaseOrgService
from allama.settings.schemas import SettingCreate, SettingUpdate, ValueType
from allama.settings.service import SettingsService
...
class AgentManagementService(BaseOrgService):
    ...
    async def get_model_config(self, model_name: str) -> ModelConfig:
        ...
        if model_name not in MODEL_CONFIGS:
            raise AllamaNotFoundError(f"Model {model_name} not found")
    ...
    async def get_provider_credential_config(
        ...
        if provider not in PROVIDER_CREDENTIAL_CONFIGS:
            raise AllamaNotFoundError(f"Provider {provider} not found")
    ...
    async def with_model_config(
        ...
        *,
        ...
        if not model_name:
            raise AllamaNotFoundError("No default model set")
