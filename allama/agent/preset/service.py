...
from allama.agent.preset.schemas import AgentPresetCreate, AgentPresetUpdate
...
from allama.integrations.enums import MCPAuthType
...
from allama.secrets.encryption import decrypt_value
...
class AgentPresetService(BaseWorkspaceService):
    ...
    async def get_agent_config_by_slug(self, slug: str) -> AgentConfig:
        ...
        raise AllamaNotFoundError(f"Agent preset with slug '{slug}' not found")
    ...
    async def get_agent_config(self, preset_id: uuid.UUID) -> AgentConfig:
        ...
        if preset := await self.get_preset(preset_id):
            return await self._preset_to_agent_config(preset)
        raise AllamaNotFoundError(f"Agent preset with ID '{preset_id}' not found")
    ...
    async def _resolve_mcp_integrations(
        ...
        encryption_key = config.ALLAMA__DB_ENCRYPTION_KEY
        if not encryption_key:
            raise AllamaValidationError(
                "ALLAMA__DB_ENCRYPTION_KEY is not set, cannot resolve MCP integrations"
            )
    ...
    async def _normalize_and_validate_slug(
        ...
        *,
        ...
        if not slug:
            raise AllamaValidationError("Agent preset slug cannot be empty")
