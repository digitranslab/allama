...
from allama.agent.types import OutputType
from allama.identifiers import UserID, WorkspaceID

# -----------------------------------------------------------------------------
# MCP Token (for tool execution)
# -----------------------------------------------------------------------------

MCP_TOKEN_ISSUER = "allama-agent-executor"
MCP_TOKEN_AUDIENCE = "allama-mcp-server"
MCP_TOKEN_SUBJECT = "allama-agent-runtime"
MCP_REQUIRED_CLAIMS = (
    "iss",
    "aud",
    "sub",
...
def mint_mcp_token(
    *,
    ...
    Returns:
        Signed JWT string
    """
    if not config.ALLAMA__SERVICE_KEY:
        raise ValueError("ALLAMA__SERVICE_KEY is not set")

    now = datetime.now(UTC)
    ttl = ttl_seconds or config.ALLAMA__EXECUTOR_TOKEN_TTL_SECONDS
    ...
    return jwt.encode(payload, config.ALLAMA__SERVICE_KEY, algorithm="HS256")
...
LLM_TOKEN_ISSUER = "allama-agent-executor"
LLM_TOKEN_AUDIENCE = "allama-llm-gateway"
LLM_TOKEN_SUBJECT = "allama-agent-runtime"
LLM_REQUIRED_CLAIMS = (
    "iss",
    "aud",
    "sub",
