#!/usr/bin/env python
"""Direct load test for executor API to reproduce queue pool overflow.

Hits the executor API endpoint to simulate concurrent load.
Run with: python scripts/benchmark/executor_load_api.py
"""
...
if not SERVICE_KEY:
    print("Error: ALLAMA__SERVICE_KEY not set in environment")
    exit(1)
...
SERVICE_HEADERS = {
    "x-allama-service-key": SERVICE_KEY,
    "x-allama-role-service-id": "allama-runner",
    "x-allama-role-access-level": "ADMIN",
    "x-allama-role-workspace-id": "7724372b-4bc9-44de-b3e0-1a7275396bfb",
}


def create_test_payload() -> dict[str, Any]:
    """Create a RunActionInput payload targeting the /wait endpoint."""
...
async def get_postgres_connections() -> dict[str, Any]:
    """Get PostgreSQL connection stats from the database."""
    import subprocess
...
async def reset_db_pool(client: httpx.AsyncClient) -> dict[str, Any]:
    """Reset/purge database pool connections."""
    try:
...
async def wait_for_pool_recovery(
    ...
    print(f"  Pool did not fully recover after {max_wait}s")
    return False
...
async def execute_single_action(
    ...
    """Execute a single action and return (index, elapsed, status_code, error)."""
    ...
    for concurrency in concurrency_levels:
        try:
            ...
            if metrics["failed"] > 0:
                ...
                if failure_rate > 50:
                    print(
                        "\n  High failure rate. Queue pool overflow likely."
                    )
                    print(
            ...
            if concurrency < concurrency_levels[-1]:
                ...
                async with httpx.AsyncClient() as recovery_client:
                    ...
                    if not recovered:
                        print(
                            "\n  Warning: DB pool still saturated. Results may be affected."
                        )
                        print(
