...
An interactive Python REPL for sending authenticated API requests to the Allama API service.
...
- `ALLAMA__PUBLIC_API_URL` - API base URL (default: `http://localhost/api`)
- `ALLAMA__TEST_USER_EMAIL` - Test user email (default: `test@allama.com`)
- `ALLAMA__TEST_USER_PASSWORD` - Test user password (default: `password1234`)
- `ALLAMA__SERVICE_KEY` - Service key for internal API calls (optional)
...
- Check that `ALLAMA__PUBLIC_API_URL` is correct
- Verify credentials with `ALLAMA__TEST_USER_EMAIL` and `ALLAMA__TEST_USER_PASSWORD`
...
- The `ALLAMA__SERVICE_KEY` must be set in your environment
