...
# ]
# ///
"""Database pool monitor with real-time terminal graphs using plotext."""
...
class DBPoolTerminalMonitor:
    ...
    def draw_graphs(self):
        # Use alternate screen buffer on first draw
        if self.first_draw:
        ...
        if x_data:
            ...
            # Use braille dots for visibility
            if len(x_data) > 1:
                plt.scatter(
                    x_data[::2],
    ...
    def get_stats_text(self) -> str:
        ...
        # ANSI box for fixed-width stats
        stats = f"""
\033[K╔══════════════════════════════════════╗
