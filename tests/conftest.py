...
# Set workflow return strategy BEFORE importing allama modules
# test_workflows.py was written when we returned the full context by default
# This must happen before any allama imports to ensure config reads the correct value
os.environ.setdefault("ALLAMA__WORKFLOW_RETURN_STRATEGY", "context")

import aioboto3
import pytest
import redis
import allama_registry.integrations.aws_boto3 as boto3_module
...
from allama.db.engine import (
    get_async_engine,
    get_async_session_context_manager,
    reset_async_engine,
)
from allama.db.models import Base, Organization, Workspace
from allama.dsl.client import get_temporal_client
from allama.dsl.plugins import AllamaPydanticAIPlugin
from allama.dsl.worker import get_activities, new_sandbox_runner
from allama.dsl.workflow import DSLWorkflow
from allama.executor.backends import ExecutorBackend
from allama.logger import logger
from allama.registry.repositories.schemas import RegistryRepositoryCreate
from allama.registry.repositories.service import RegistryReposService
from allama.secrets import secrets_manager
...
MINIO_WORKFLOW_BUCKET = "test-allama-workflow"
...
TEMPORAL_TASK_QUEUE = f"allama-task-queue-{WORKER_ID}"
...
def registry_version_with_manifest(db: None, env_sandbox: None) -> Iterator[None]:
    ...
    def _seed_registry_version(sync_db_uri: str) -> None:
        ...
        with Session(sync_engine) as session:
            ...
            core_transform_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.core.transform",
                "name": "reshape",
            }
            ...
            http_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.core.http",
                "name": "http_request",
            }
            ...
            wf_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.core.workflow",
                "name": "execute",
            }
            ...
            email_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.core.email",
                "name": "send_email",
            }
            ...
            open_case_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.core.cases",
                "name": "open_case",
            }
            ...
            table_lookup_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.core.table",
                "name": "lookup",
            }
            ...
            script_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.core.script",
                "name": "run_python",
            }
            ...
            ai_extract_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.core.ai",
                "name": "extract",
            }
            ...
            webhook_impl = {
                "type": "udf",
                "url": origin,
                "module": "allama_registry.integrations.sinks",
                "name": "webhook",
            }
            ...
            # The executor routes allama_registry origin to platform tables
            ...
            from allama.db.models import PlatformRegistryIndex
    ...
    target_uris = {TEST_DB_CONFIG.test_url_sync, config.ALLAMA__DB_URI}
...
def env_sandbox(monkeysession: pytest.MonkeyPatch):
    ...
    monkeysession.setattr(config, "ALLAMA__APP_ENV", "development")
    ...
    monkeysession.setattr(config, "ALLAMA__DB_URI", db_uri)
    ...
    monkeysession.setattr(
        config, "ALLAMA__BLOB_STORAGE_ENDPOINT", blob_storage_endpoint
    )
    # Configure MinIO for result externalization (StoredObject -> S3)
    monkeysession.setattr(
        config, "ALLAMA__BLOB_STORAGE_BUCKET_WORKFLOW", MINIO_WORKFLOW_BUCKET
    )
    monkeysession.setattr(config, "ALLAMA__RESULT_EXTERNALIZATION_ENABLED", True)
    # Externalize all results for testing (threshold=0)
    monkeysession.setattr(config, "ALLAMA__RESULT_EXTERNALIZATION_THRESHOLD_BYTES", 0)
    monkeysession.setattr(config, "ALLAMA__AUTH_ALLOWED_DOMAINS", ["allama.com"])
    if os.getenv("ALLAMA__CONTEXT_COMPRESSION_ENABLED"):
        logger.info("Enabling compression for workflow context")
        monkeysession.setattr(config, "ALLAMA__CONTEXT_COMPRESSION_ENABLED", True)
        # Force compression for local unit tests
        monkeysession.setattr(config, "ALLAMA__CONTEXT_COMPRESSION_THRESHOLD_KB", 0)
    ...
    monkeysession.setenv("ALLAMA__DB_URI", db_uri)
    monkeysession.setenv("ALLAMA__BLOB_STORAGE_ENDPOINT", blob_storage_endpoint)
    monkeysession.setenv(
        "ALLAMA__BLOB_STORAGE_BUCKET_WORKFLOW", MINIO_WORKFLOW_BUCKET
    )
    monkeysession.setenv("ALLAMA__RESULT_EXTERNALIZATION_ENABLED", "true")
    monkeysession.setenv("ALLAMA__RESULT_EXTERNALIZATION_THRESHOLD_BYTES", "0")
    # monkeysession.setenv("ALLAMA__DB_ENCRYPTION_KEY", Fernet.generate_key().decode())
    ...
    monkeysession.setattr(config, "ALLAMA__API_URL", api_url)
    monkeysession.setenv("ALLAMA__API_URL", api_url)
    monkeysession.setenv("ALLAMA__EXECUTOR_URL", executor_url)
    # Use DirectBackend for in-process executor (no sandbox overhead) unless overridden
    if not IN_DOCKER:
        monkeysession.setattr(config, "ALLAMA__EXECUTOR_BACKEND", "direct")
        monkeysession.setenv("ALLAMA__EXECUTOR_BACKEND", "direct")
    monkeysession.setenv("ALLAMA__PUBLIC_API_URL", f"http://{api_host}/api")
    monkeysession.setenv("ALLAMA__SERVICE_KEY", os.environ["ALLAMA__SERVICE_KEY"])
    monkeysession.setenv("ALLAMA__SIGNING_SECRET", "test-signing-secret")
    ...
    monkeysession.setenv("ALLAMA__EXECUTOR_QUEUE", EXECUTOR_TASK_QUEUE)
    monkeysession.setattr(config, "ALLAMA__EXECUTOR_QUEUE", EXECUTOR_TASK_QUEUE)
    monkeysession.setenv("ALLAMA__AGENT_QUEUE", AGENT_TASK_QUEUE)
    monkeysession.setattr(config, "ALLAMA__AGENT_QUEUE", AGENT_TASK_QUEUE)
...
def temporal_client():
    ...
    client = loop.run_until_complete(
        get_temporal_client(plugins=[AllamaPydanticAIPlugin()])
    )
    return client
    ...
    async with RegistryReposService.with_session(role=test_role) as svc:
        db_repo = await svc.create_repository(
            RegistryRepositoryCreate(
                origin=f"git+ssh://git@github.com/digitranslab/dummy-repo-{uuid.uuid4().hex[:8]}.git"
            )
        )
        try:
...
def workflow_bucket(minio_server, env_sandbox):
    ...
    """
    from allama.storage import blob
    from allama.storage import object as object_module
...
async def executor_backend() -> AsyncGenerator[ExecutorBackend, None]:
    ...
    from allama.executor.backends import (
...
async def test_executor_worker_factory(
    ...
    """
    from allama.executor.activities import ExecutorActivities
    ...
    def create_worker(
        ...
        *,
        ...
        return Worker(
            client=client,
            task_queue=task_queue or config.ALLAMA__EXECUTOR_QUEUE,
...
# Loaded in either via dotenv or env vars into the mocked Allama secrets manager
# ---------------------------------------------------------------------------
...
def mock_openai_secrets(monkeypatch: pytest.MonkeyPatch):
    ...
    with (
        patch("allama_registry._internal.secrets.get") as mock_get,
...
async def mock_agent_session(session: AsyncSession, svc_role: Role):
    ...
    """
    from allama.db.models import AgentSession
