...
from allama.storage import blob as blob_module
from allama.storage.blob import (
...
class TestS3Operations:
    ...
    @pytest.mark.anyio
    @patch("allama.storage.blob.get_storage_client")
    ...
    async def test_get_storage_client_minio_uses_endpoint_and_env(self, monkeypatch):
        ...
        with patch("allama.storage.blob.aioboto3.Session") as mock_session_cls:
    ...
    @pytest.mark.anyio
    @patch("allama.storage.blob.get_storage_client")
    @patch("allama.storage.blob.config")
    async def test_generate_presigned_download_url(self, mock_config, mock_get_client):
        """Test presigned download URL generation."""
        # Mock config
        mock_config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_EXPIRY = 10
        mock_config.ALLAMA__PUBLIC_APP_URL = "http://localhost"
        mock_config.ALLAMA__BLOB_STORAGE_ENDPOINT = "http://minio:9000"
        mock_config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_ENDPOINT = None
    ...
    async def test_presigned_url_path_replacement(self, mock_config, mock_get_client):
        ...
        mock_config.ALLAMA__PUBLIC_APP_URL = "http://localhost"
        mock_config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_EXPIRY = 10
        mock_config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_ENDPOINT = (
            "http://localhost/s3"
        )
        mock_config.ALLAMA__BLOB_STORAGE_ENDPOINT = "http://minio:9000"
        ...
        original_url = "http://minio:9000/allama/attachments/test.txt?AWSAccessKeyId=minio&Signature=abc123&Expires=1234567890"
        mock_client.generate_presigned_url.return_value = original_url

        # Generate presigned URL
        result = await generate_presigned_download_url(
            "attachments/test.txt", "allama", 30
        )

        # Verify URL transformation
        expected_url = "http://localhost/s3/allama/attachments/test.txt?AWSAccessKeyId=minio&Signature=abc123&Expires=1234567890"
        assert result == expected_url
        assert "/s3/" in result
    ...
    async def test_presigned_url_no_replacement_without_blob_endpoint(
        ...
        mock_config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_EXPIRY = 10
        mock_config.ALLAMA__BLOB_STORAGE_ENDPOINT = None
        mock_config.ALLAMA__BLOB_STORAGE_PRESIGNED_URL_ENDPOINT = "http://public/s3"
...
class TestEdgeCases:
    ...
    @patch("allama.storage.blob.logger")
    @pytest.mark.anyio
    @patch("allama.storage.blob.get_storage_client")
    ...
    async def test_download_file_to_path_writes_bytes(
        ...
        monkeypatch.setattr(
            "allama.storage.blob.open_download_stream",
