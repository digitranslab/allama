...
from allama.cases.enums import CaseEventType, CasePriority, CaseSeverity, CaseStatus
from allama.cases.schemas import CaseCreate
from allama.cases.service import CaseEventsService, CasesService
from allama.exceptions import AllamaAuthorizationError, AllamaException
from allama.storage.blob import ensure_bucket_exists
...
async def configure_minio_for_attachments(
    ...
    monkeypatch.setattr(
        config, "ALLAMA__BLOB_STORAGE_BUCKET_ATTACHMENTS", minio_bucket, raising=False
    )
...
async def test_max_attachments_limit_enforced(
    ...
    monkeypatch.setattr(config, "ALLAMA__MAX_ATTACHMENTS_PER_CASE", 1, raising=False)
...
async def test_storage_limit_enforced(
    ...
    monkeypatch.setattr(
        config,
        "ALLAMA__MAX_CASE_STORAGE_BYTES",
...
async def test_integrity_check_failure_after_object_corruption(
    ...
    with pytest.raises(AllamaException, match="File integrity check failed"):
