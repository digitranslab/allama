...
from allama.agent.context import (
...
def test_truncate_tool_returns_processor_no_truncation_needed():
    ...
    with patch("allama.agent.context.ALLAMA__AGENT_TOOL_OUTPUT_LIMIT", 1000):
...
def test_truncate_tool_returns_processor_truncates_large_output():
    ...
    with patch("allama.agent.context.ALLAMA__AGENT_TOOL_OUTPUT_LIMIT", 100):
        result = truncate_tool_returns_processor(
            mock_ctx, cast(list[ModelMessage], messages)
        )
...
def test_trim_history_processor_under_limit():
    ...
    with patch(
        "allama.agent.context.ALLAMA__MODEL_CONTEXT_LIMITS", {"test-model": 100000}
    ):
...
def test_trim_history_processor_trims_over_limit():
    ...
    with patch(
        "allama.agent.context.ALLAMA__MODEL_CONTEXT_LIMITS", {"test-model": 500}
    ):
        result = trim_history_processor(mock_ctx, messages)
...
def test_trim_history_processor_uses_default_limit():
    ...
    with patch("allama.agent.context.ALLAMA__MODEL_CONTEXT_LIMITS", {}):
        with patch(
            "allama.agent.context.ALLAMA__AGENT_DEFAULT_CONTEXT_LIMIT", 100000
        ):
            result = trim_history_processor(mock_ctx, messages)
