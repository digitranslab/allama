...
from allama.registry.actions.schemas import RegistryActionType
...
class TestAgentPresetService:
    ...
    async def test_create_preset_with_invalid_actions(
        ...
        with pytest.raises(
            AllamaValidationError, match="2 actions were not found in the registry"
        ):
    ...
    async def test_empty_slug_raises_error(
        ...
        with pytest.raises(
            AllamaValidationError, match="Agent preset slug cannot be empty"
        ):
            await agent_preset_service.create_preset(agent_preset_create_params)
    ...
    async def test_get_agent_config_not_found(
        ...
        with pytest.raises(
            AllamaNotFoundError, match="Agent preset with ID .* not found"
        ):
