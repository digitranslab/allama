        ...
        manifest = _make_manifest(
            {
                "core.transform.reshape": {
                    "type": "udf",
                    "url": "allama_registry",
                    "module": "allama_registry.integrations.core.transform",
                    "name": "reshape",
                }
            }
        )

        index = registry_resolver._build_impl_index(manifest, "allama_registry")

        assert "core.transform.reshape" in index
        impl = index["core.transform.reshape"]
        assert impl.type == "udf"
        assert impl.module == "allama_registry.integrations.core.transform"
        assert impl.name == "reshape"
        assert impl.origin == "allama_registry"
...
class TestResolveAction:
    ...
    async def test_resolve_action_success(self):
        ...
        impl_index = registry_resolver._build_impl_index(manifest, "allama_registry")

        lock = RegistryLock(
            origins={"allama_registry": "2024.12.10"},
            actions={"core.transform.reshape": "allama_registry"},
        )
        ...
        assert action_impl.module == "allama_registry.integrations.core.transform"
        assert action_impl.origin == "allama_registry"
...
class TestCollectActionSecretsFromManifest:
    ...
    async def test_collect_udf_secrets(self):
        ...
        manifest = _make_manifest(
            {
                "tools.api.call": {
                    "type": "udf",
                    "url": "allama_registry",
                    "module": "allama_registry.integrations.tools.api",
                    "name": "call",
                }
            }
        )
        ...
        lock = RegistryLock(
            origins={"allama_registry": "2024.12.10"},
            actions={"tools.api.call": "allama_registry"},
        )
