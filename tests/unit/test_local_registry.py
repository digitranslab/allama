...
from allama.registry.constants import DEFAULT_LOCAL_REGISTRY_ORIGIN
...
def sample_package_content() -> str:
    ...
    return dedent(
        """
        from allama_registry import registry
...
async def test_local_registry(
    ...
    monkeypatch.setattr(config, "ALLAMA__LOCAL_REPOSITORY_ENABLED", True)
    monkeypatch.setattr(
        config,
        "ALLAMA__LOCAL_REPOSITORY_CONTAINER_PATH",
...
async def test_local_registry_invalid_package(
    ...
    monkeypatch.setenv("ALLAMA__LOCAL_REPOSITORY_CONTAINER_PATH", str(invalid_path))
    ...
    with pytest.raises(
        ...
        " Please set ALLAMA__LOCAL_REPOSITORY_ENABLED=true and ensure"
        " ALLAMA__LOCAL_REPOSITORY_PATH points to a valid Python package.",
    ):
