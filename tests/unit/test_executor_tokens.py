...
from allama.auth import credentials
from allama.auth.credentials import _role_dependency
from allama.auth.executor_tokens import (
...
def test_mint_and_verify_executor_token_roundtrip(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(config, "ALLAMA__SERVICE_KEY", "test-service-key")
...
def test_verify_executor_token_invalid_subject(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(config, "ALLAMA__SERVICE_KEY", service_key)
...
async def test_role_dependency_executor_token_derives_access_level_from_db(
    ...
    assert resolved.service_id == "allama-executor"
