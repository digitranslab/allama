...
from allama_registry import RegistryOAuthSecret, registry
...
from allama.executor.service import dispatch_action
...
from allama.registry.actions.service import (
...
from allama.validation.schemas import ActionValidationResult, ValidationResultType
...
async def test_template_action_with_optional_oauth_both_ac_and_cc(
    ...
    monkeysession.setattr("allama.config.ALLAMA__UNSAFE_DISABLE_SM_MASKING", True)
    ...
    assert exc_info.value.info.type == "AllamaCredentialsError"
...
async def test_agent_tool_approvals_requires_feature_flag(
    ...
    monkeypatch.setattr(config, "ALLAMA__FEATURE_FLAGS", set())
