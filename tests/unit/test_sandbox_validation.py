...
from allama.sandbox.exceptions import SandboxValidationError
from allama.sandbox.executor import (
    NsjailExecutor,
    _validate_cache_key,
    _validate_env_key,
    _validate_path,
)
from allama.sandbox.types import SandboxConfig
...
class TestEnvMap:
    ...
    def test_env_map_does_not_inherit_host_env(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setenv("ALLAMA__DB_URI", "secret")
        ...
        assert "ALLAMA__DB_URI" not in env_map
        assert env_map["PATH"] == "/usr/local/bin:/usr/bin:/bin"

    def test_no_allama_vars_leak(self, monkeypatch: pytest.MonkeyPatch):
        """Verify NO ALLAMA__ prefixed vars are included in env_map.

        This is a comprehensive check to ensure no secrets configured via
        ALLAMA__ environment variables leak into the sandbox.
        """
        # Set multiple ALLAMA__ vars to simulate real environment
        monkeypatch.setenv("ALLAMA__DB_URI", "postgresql://secret:secret@db/allama")
        monkeypatch.setenv("ALLAMA__API_KEY", "sk-secret-key-12345")
        monkeypatch.setenv("ALLAMA__SERVICE_KEY", "service-secret")
        monkeypatch.setenv("ALLAMA__SIGNING_SECRET", "hmac-signing-secret")
        monkeypatch.setenv("ALLAMA__ENCRYPTION_KEY", "encryption-key-secret")
        ...
        # Ensure NO ALLAMA__ vars are present
        allama_vars = [k for k in env_map.keys() if k.startswith("ALLAMA__")]
        assert allama_vars == [], f"Found ALLAMA__ vars in env_map: {allama_vars}"
