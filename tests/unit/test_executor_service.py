...
async def test_get_action_secrets_passes_sets_to_auth_sandbox(mocker):
    ...
    mocker.patch(
        "allama.expressions.eval.extract_templated_secrets",
        return_value=["args_secret1", "args_secret2"],
    )
    mocker.patch(
        "allama.secrets.secrets_manager.get_runtime_env", return_value="test_env"
    )
    ...
    auth_sandbox_mock = mocker.patch("allama.secrets.secrets_manager.AuthSandbox")
...
async def test_get_action_secrets_skips_optional_oauth(mocker):
    ...
    mocker.patch("allama.expressions.eval.extract_templated_secrets", return_value=[])
    mocker.patch(
        "allama.secrets.secrets_manager.get_runtime_env", return_value="test_env"
    )
    ...
    mocker.patch("allama.secrets.secrets_manager.AuthSandbox", return_value=sandbox)
    ...
    mocker.patch(
        "allama.secrets.secrets_manager.IntegrationService.with_session",
...
async def test_extract_templated_secrets_detects_nested_complex_expressions():
    from allama.expressions.eval import extract_templated_secrets
