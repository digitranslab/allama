...
from allama.agent.common.protocol import RuntimeInitPayload
from allama.agent.common.socket_io import SocketStreamWriter
from allama.agent.common.stream_types import StreamEventType, UnifiedStreamEvent
...
from allama.agent.runtime.claude_code.runtime import ClaudeAgentRuntime
...
class TestClaudeAgentRuntimeRun:
    ...
    async def test_sends_done_on_completion(
        ...
        with (
            patch(
                "allama.agent.runtime.claude_code.runtime.ClaudeSDKClient",
                return_value=mock_claude_sdk_client,
            ),
            patch(
                "allama.agent.runtime.claude_code.runtime.create_proxy_mcp_server",
    ...
    async def test_handles_stream_events(
        ...
        with (
            ...
            patch(
                "allama.agent.runtime.claude_code.runtime.ClaudeSDKAdapter",
                return_value=mock_adapter,
            ),
            patch("allama.agent.runtime.claude_code.runtime.StreamEvent", MagicMock),
        ):
            runtime = ClaudeAgentRuntime(mock_socket_writer)
            await runtime.run(sample_init_payload)
...
class TestClaudeAgentRuntimePreToolUseHook:
    ...
    1. Auto-approve if it's a user MCP tool (starts with mcp__ but not mcp__allama-registry__)
    ...
    async def test_auto_approve_user_mcp_tools(
        ...
        result = await runtime._pre_tool_use_hook(
            input_data=make_hook_input(
                tool_name="mcp__allama-registry__mcp__some_tool",
    ...
    async def test_auto_approve_registry_tool_without_approval(
        ...
        result = await runtime._pre_tool_use_hook(
            input_data=make_hook_input(
                # MCP tool name format: mcp__allama-registry__core__http_request
                tool_name="mcp__allama-registry__core__http_request",
