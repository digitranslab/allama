...
from allama.agent import router as agent_router
...
async def test_get_provider_credential_config_not_found(
    ...
    with patch.object(agent_router, "AgentManagementService") as MockService:
        mock_svc = AsyncMock()
        mock_svc.get_provider_credential_config.side_effect = AllamaNotFoundError(
            "Provider not found"
        )
        MockService.return_value = mock_svc
...
async def test_update_provider_credentials_not_found(
    ...
    with patch.object(agent_router, "AgentManagementService") as MockService:
        mock_svc = AsyncMock()
        mock_svc.update_provider_credentials.side_effect = AllamaNotFoundError(
...
async def test_set_default_model_not_found(
    ...
    with patch.object(agent_router, "AgentManagementService") as MockService:
        mock_svc = AsyncMock()
        mock_svc.set_default_model.side_effect = AllamaNotFoundError(
            "Model not found"
        )
        MockService.return_value = mock_svc
