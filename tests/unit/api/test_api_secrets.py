...
from allama.db.models import OrganizationSecret, Secret, Workspace
from allama.exceptions import AllamaNotFoundError
from allama.secrets import router as secrets_router
from allama.secrets.enums import SecretType
from allama.secrets.schemas import SecretDefinition, SecretKeyValue
...
async def test_get_secret_by_name_not_found(
    ...
    with patch.object(secrets_router, "SecretsService") as MockService:
        mock_svc = AsyncMock()
        mock_svc.get_secret_by_name.side_effect = AllamaNotFoundError(
            "Secret not found"
        )
        MockService.return_value = mock_svc
...
async def test_update_secret_by_id_not_found(
    ...
    with patch.object(secrets_router, "SecretsService") as MockService:
        mock_svc = AsyncMock()
        mock_svc.get_secret.side_effect = AllamaNotFoundError("Secret not found")
        MockService.return_value = mock_svc
