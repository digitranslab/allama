...
from allama.sandbox.safe_executor import SafePythonExecutor, _extract_package_name
...
class TestSafeExecutorFallback:
    ...
    These tests run with ALLAMA__DISABLE_NSJAIL=true to use the safe executor
    ...
    def force_safe_executor(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("ALLAMA__SANDBOX_CACHE_DIR", cache_dir)
        # Reload config to pick up the change
        import allama.config as config_module
    ...
    def sandbox_service(self, tmp_path):
        ...
        from allama.sandbox import SandboxService
