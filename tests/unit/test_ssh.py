...
class TestGetGitSshCommand:
    ...
    async def test_get_git_ssh_command(self):
        ...
        role = Role(type="service", service_id="allama-service")
        ...
        with (
            patch("allama.ssh.SecretsService", return_value=mock_service),
            patch("allama.ssh.prepare_ssh_key_file", return_value=expected_ssh_cmd),
        ):
    ...
    async def test_get_git_ssh_command_with_context_role(self):
        ...
        mock_role = Role(type="service", service_id="allama-service")
        with (
            patch("allama.ssh.ctx_role") as mock_ctx_role,
            patch("allama.ssh.SecretsService", return_value=mock_service),
            patch("allama.ssh.prepare_ssh_key_file", return_value=expected_ssh_cmd),
        ):
...
class TestAddHostToKnownHosts:
    ...
    def test_split_host_port(
        ...
        from allama.ssh import _split_host_port
