"""
Reproduction harness for the historical import/reload race around `allama_registry`.
...
from allama.registry.repository import import_and_reload as safe_import_and_reload
...
async def test_import_reload_race_old_behavior(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    importlib.import_module("allama_registry")

    # Monkeypatch to the bad importer
    monkeypatch.setattr(
        "allama.registry.repository.import_and_reload",
    ...
    async def reloader_task(duration_s: float = 1.0) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            await asyncio.to_thread(_bad_import_and_reload, "allama_registry")
    ...
    async def importer_task(duration_s: float = 1.0) -> None:
        ...
        while time.monotonic() < deadline:
            try:
                ...
                sys.modules.pop(
                    "allama_registry.integrations.crowdstrike_falconpy", None
                )
                sys.modules.pop("allama_registry.integrations", None)
                importlib.invalidate_caches()
                importlib.import_module(
                    "allama_registry.integrations.crowdstrike_falconpy"
                )
...
async def test_import_reload_no_race_with_lock() -> None:
    ...
    async def reloader_task(iterations: int = 200) -> None:
        for _ in range(iterations):
            ...
            await asyncio.to_thread(safe_import_and_reload, "allama_registry")
