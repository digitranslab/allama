...
async def test_startup_sync_skips_when_version_already_current(
    ...
    from allama.registry.sync.jobs import _sync_as_leader
    ...
    # Mock allama_registry.__version__ to return our version
    with patch("allama.registry.sync.jobs.allama_registry") as mock_registry:
        mock_registry.__version__ = "0.1.0"

        # Should complete without calling sync
        with patch(
            "allama.registry.sync.jobs.PlatformRegistrySyncService"
...
async def test_startup_sync_leader_election_non_leader_exits(
    ...
    with (
        patch(
            "allama.registry.sync.jobs.try_pg_advisory_lock", return_value=False
        ) as mock_lock,
        patch("allama.registry.sync.jobs._sync_as_leader") as mock_sync,
    ):
