...
from allama.db.models import OAuthIntegration
from allama.exceptions import AllamaAuthorizationError
from allama.integrations.enums import OAuthGrantType
from allama.integrations.providers.base import (
    AuthorizationCodeOAuthProvider,
    ClientCredentialsOAuthProvider,
)
from allama.integrations.schemas import (
    IntegrationUpdate,
    ProviderConfig,
    ProviderKey,
    ProviderMetadata,
...
from allama.integrations.service import (
    InsecureOAuthEndpointError,
    IntegrationService,
)
from allama.integrations.types import TokenResponse
...
def encryption_key(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    monkeypatch.setenv("ALLAMA__DB_ENCRYPTION_KEY", key)
    return key
...
def mock_provider(monkeypatch: pytest.MonkeyPatch) -> MockOAuthProvider:
    ...
    monkeypatch.setattr("allama.config.ALLAMA__PUBLIC_APP_URL", "http://localhost")
...
class TestIntegrationService:
    ...
    async def test_refresh_token_if_not_needed(
        ...
        with patch(
            "allama.integrations.service.get_provider_class"
    ...
    async def test_encryption_key_mismatch(
        ...
        with patch.dict(os.environ, {"ALLAMA__DB_ENCRYPTION_KEY": key1}):
        ...
        with patch.dict(os.environ, {"ALLAMA__DB_ENCRYPTION_KEY": key2}):
...
class TestBaseOAuthProvider:
    ...
    async def test_get_authorization_url_with_pkce(
        ...
        with (
            patch(
                "allama.integrations.providers.base.secrets.token_urlsafe",
    ...
    async def test_from_config(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "allama.config.ALLAMA__PUBLIC_APP_URL", "http://localhost"
        )
