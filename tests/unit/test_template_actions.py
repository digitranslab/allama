...
from allama.db.models import RegistryRepository, RegistryVersion
...
from allama.executor import service
    ...
    try:
        ...
        with open(os.path.join(tmp_path, "has_secret.py"), "w") as f:
            f.write(
                textwrap.dedent(
                    """
                from allama_registry import registry, RegistrySecret, secrets
...
def test_template_action_definition_validates_self_reference():
    ...
    """
    with pytest.raises(AllamaValidationError) as exc_info:
        TemplateActionDefinition(
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"user_id": "john@allama.com", "service_source": "custom", "limit": 99},
            ["john@allama.com", "custom", 99],
            False,
        ),
        (
            {"user_id": "john@allama.com"},
            ["john@allama.com", "elastic", 100],
            False,
        ),
        (
            {},
...
def test_aggregate_secrets_from_manifest_collects_nested_secrets():
    ...
    from allama.registry.actions.service import RegistryActionsService
    from allama.registry.versions.schemas import (
