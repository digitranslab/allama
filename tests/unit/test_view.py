...
from allama.dsl.enums import EdgeType
from allama.dsl.schemas import ActionStatement
from allama.dsl.view import RFEdge, RFGraph, TriggerNode, TriggerNodeData, UDFNode
from allama.exceptions import AllamaValidationError

if TYPE_CHECKING:
    from allama.db.models import Action, Workflow
...
def test_validate_graph_empty_nodes():
    ...
    with pytest.raises(
        AllamaValidationError, match="Graph must have at least one node"
    ):
...
def test_validate_graph_no_trigger():
    ...
    with pytest.raises(AllamaValidationError, match="Expected 1 trigger node, got 0"):
...
def test_validate_graph_multiple_triggers():
    ...
    with pytest.raises(AllamaValidationError, match="Expected 1 trigger node, got 2"):
        RFGraph.model_validate(rf_obj)
