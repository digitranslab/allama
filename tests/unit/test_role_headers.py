...
from allama.auth.credentials import _authenticate_service
...
class TestRoleToHeaders:
    ...
    def test_to_headers_includes_workspace_role_editor(self):
        ...
        assert "x-allama-role-workspace-role" in headers
        assert headers["x-allama-role-workspace-role"] == "editor"
    ...
    def test_to_headers_includes_workspace_role_admin(self):
        ...
        headers = role.to_headers()
        assert "x-allama-role-workspace-role" in headers
        assert headers["x-allama-role-workspace-role"] == "admin"
    ...
    def test_to_headers_excludes_workspace_role_when_none(self):
        ...
        headers = role.to_headers()
        assert "x-allama-role-workspace-role" not in headers
    ...
    def test_to_headers_roundtrip_preserves_workspace_role(self):
        ...
        assert headers["x-allama-role-type"] == "service"
        assert headers["x-allama-role-service-id"] == "allama-runner"
        assert headers["x-allama-role-access-level"] == "BASIC"
        assert headers["x-allama-role-user-id"] == str(user_id)
        assert headers["x-allama-role-workspace-id"] == str(workspace_id)
        assert headers["x-allama-role-workspace-role"] == "editor"
...
class TestAuthenticateServiceWorkspaceRole:
    ...
    async def test_authenticate_service_reads_workspace_role_editor(
        ...
        monkeypatch.setattr(
            "allama.auth.credentials.config.ALLAMA__SERVICE_KEY", "test-key"
        )
        monkeypatch.setattr(
            "allama.auth.credentials.config.ALLAMA__SERVICE_ROLES_WHITELIST",
            ["allama-runner"],
        )
        ...
        request.headers = MockHeaders(
            {
                "x-allama-role-service-id": "allama-runner",
                "x-allama-role-access-level": "BASIC",
                "x-allama-role-workspace-id": workspace_id,
                "x-allama-role-workspace-role": "editor",
            }
        )
        ...
        request.headers = MockHeaders(
            {
                ...
                "x-allama-role-access-level": "BASIC",
                "x-allama-role-workspace-id": workspace_id,
                "x-allama-role-workspace-role": "admin",
            }
        )
