...
from allama.db.models import Workspace
from allama.identifiers.workflow import WorkflowExecutionID
...
from allama.workflow.executions.service import (
    WF_FAILURE_REF,
...
class TestWorkflowExecutionEvents:
    ...
    async def test_workflow_failure_synthetic_event_creation(
        ...
        with patch(
            "allama.workflow.executions.service.EventFailure.from_history_event"
    ...
    async def test_workflow_execution_without_failures_no_synthetic_events(
        ...
        with patch(
            "allama.workflow.executions.service.WorkflowExecutionEventCompact.from_source_event"
        ) as mock_from_source:
            ...
            with patch(
                "allama.workflow.executions.service.get_source_event_id"
            ) as mock_get_source:
                mock_get_source.return_value = 1

                with patch(
                    "allama.workflow.executions.service.is_start_event"
                ) as mock_is_start:
                    with patch(
                        "allama.workflow.executions.service.is_close_event"
                    ) as mock_is_close:
                        with patch(
                            "allama.workflow.executions.service.is_error_event"
                        ) as mock_is_error:
                            ...
                            with patch(
                                "allama.workflow.executions.service.get_result"
                            ) as mock_get_result:
