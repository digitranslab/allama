...
from allama.auth.credentials import TemporaryRole
from allama.auth.sandbox import AuthSandbox
from allama.auth.types import Role
from allama.contexts import ctx_role, get_env
...
from allama.secrets.encryption import encrypt_keyvalues
...
async def test_auth_sandbox_with_secrets(mocker: pytest_mock.MockFixture, test_role):
    ...
    mock_secret = Secret(
        ...
        encrypted_keys=encrypt_keyvalues(
            mock_secret_keys, key=os.environ["ALLAMA__DB_ENCRYPTION_KEY"]
        ),
...
async def test_auth_sandbox_without_secrets(test_role, mock_user_id):
    ...
    with TemporaryRole(
        type="service", user_id=mock_user_id, service_id="allama-service"
    ):
...
async def test_auth_sandbox_missing_secret(mocker: pytest_mock.MockFixture, test_role):
    ...
    mocker.patch(
        "allama.auth.sandbox.SecretsService.with_session",
        return_value=mocker.AsyncMock(
            __aenter__=mocker.AsyncMock(return_value=mock_secrets_service)
        ),
    )

    with pytest.raises(AllamaCredentialsError):
...
async def test_auth_sandbox_optional_secrets(
    ...
    required_secret = Secret(
        ...
        encrypted_keys=encrypt_keyvalues(
            ...
            key=os.environ["ALLAMA__DB_ENCRYPTION_KEY"],
        ),
        created_at=datetime.now(),
        updated_at=datetime.now(),
        tags={},
    ...
    mock_secrets_service.search_secrets.return_value = []
    with pytest.raises(AllamaCredentialsError) as exc_info:
        async with AuthSandbox(
            secrets=["required_secret", "optional_secret"],
            optional_secrets=["optional_secret"],
        ):
