...
from allama.cases.enums import CasePriority, CaseSeverity, CaseStatus
from allama.cases.schemas import CaseCreate, CaseFieldCreate, CaseUpdate
from allama.cases.service import CaseFieldsService, CasesService
...
async def test_service_initialization_requires_workspace(session: AsyncSession) -> None:
    ...
    with pytest.raises(AllamaAuthorizationError):
...
class TestCasesService:
    ...
    async def test_create_and_get_case_with_assignee(
        ...
        with patch(
            "allama.cases.service.CasesService.create_case"
        ) as mock_create_case:
    ...
    async def test_update_case_with_assignee(
        ...
        with patch("allama.cases.service.CasesService.create_case") as mock_create:
        ...
        with patch("allama.cases.service.CasesService.update_case") as mock_update:
    ...
    async def test_create_case_fields_update_fails(
        ...
        # Mock update_row to raise AllamaNotFoundError (simulating UPDATE matching 0 rows)
        mocker.patch.object(
            cases_service.fields.editor,
            "update_row",
            side_effect=AllamaNotFoundError("Row not found in table case_fields"),
        )

        # Should raise AllamaException with helpful message
        with pytest.raises(AllamaException) as exc_info:
