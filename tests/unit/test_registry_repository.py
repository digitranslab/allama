...
from allama.registry.repository import (
    RegisterKwargs,
    attach_validators,
...
def test_import_and_reload_falls_back_when_loader_missing(
    ...
    module_name = "allama._test_dummy_registry"
    ...
    monkeypatch.setattr(
        "allama.registry.repository.importlib.import_module", fake_import
    )

    def boom(_: ModuleType) -> ModuleType:  # pragma: no cover - should not run
        raise AssertionError("reload should not have been called")

    monkeypatch.setattr("allama.registry.repository.importlib.reload", boom)
...
def test_generate_model_preserves_component_metadata() -> None:
    """Ensure `x-allama-component` propagates to JSON schema.

    Registry modules use lightweight dataclasses from `allama_registry.fields`
    which are not instances of `allama.registry.fields.Component` but should
    still propagate to JSON schema so the UI can render specialized editors.
    """
    import allama_registry.fields as registry_fields
    ...
    components = script_schema.get("x-allama-component", [])
...
def test_registry_module_with_future_annotations_resolves_types() -> None:
    ...
    """
    from allama_registry.core.table import lookup
...
def test_registry_module_literal_types_preserved() -> None:
    ...
    """
    from allama_registry.core.table import download
