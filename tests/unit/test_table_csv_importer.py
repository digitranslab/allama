...
from allama.exceptions import AllamaImportError
from allama.tables.enums import SqlType
from allama.tables.importer import ColumnInfo, CSVImporter, CSVSchemaInferer
...
class TestCSVSchemaInferer:
    ...
    def test_rejects_duplicate_headers(self) -> None:
        ...
        with pytest.raises(AllamaImportError, match="Duplicate columns: Name"):
            CSVSchemaInferer.initialise(headers)

    def test_rejects_blank_duplicate_headers(self) -> None:
        headers = ["", " ", ""]
        with pytest.raises(AllamaImportError, match="Duplicate columns: <empty>"):
            CSVSchemaInferer.initialise(headers)
