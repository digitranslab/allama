...
from allama.auth.credentials import RoleACL, _role_dependency
...
from allama.authz.service import MembershipWithOrg
from allama.db.models import Membership, Organization, User, Workspace
from allama.middleware import AuthorizationCacheMiddleware
...
async def test_auth_cache_reduces_database_queries(mocker):
    ...
    mocker.patch(
        "allama.auth.credentials.MembershipService", return_value=mock_service
    )

    # Mock the access level lookup
    mocker.patch.dict(
        "allama.auth.credentials.USER_ROLE_TO_ACCESS_LEVEL",
        {UserRole.BASIC: AccessLevel.BASIC},
    )

    # Mock is_unprivileged to return True for basic users
    mocker.patch("allama.auth.credentials.is_unprivileged", return_value=True)
...
async def test_cache_user_id_validation():
    ...
    # Set up mocks
    with (
        patch("allama.auth.credentials.MembershipService", return_value=mock_service),
        patch("allama.auth.credentials.is_unprivileged", return_value=True),
        patch.dict(
            "allama.auth.credentials.USER_ROLE_TO_ACCESS_LEVEL",
            {UserRole.BASIC: AccessLevel.BASIC},
        ),
...
async def test_organization_id_populated_when_require_workspace_no(mocker):
    ...
    mocker.patch("allama.auth.credentials.is_unprivileged", return_value=False)
    ...
    mocker.patch(
        "allama.auth.credentials.MembershipService.list_user_memberships_with_org",
