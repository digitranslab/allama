...
class TestExecuteActionActivity:
    ...
    async def test_successful_execution(self, mock_run_action_input, mock_role):
        ...
        with (
            patch("allama.executor.activities.activity") as mock_activity,
            patch("allama.executor.activities.get_executor_backend") as mock_backend,
            patch(
                "allama.executor.activities.dispatch_action",
                new_callable=AsyncMock,
            ) as mock_dispatch,
            patch(
                "allama.executor.activities.materialize_context",
                new_callable=AsyncMock,
            ) as mock_materialize,
        ):
    ...
    async def test_context_variables_are_set(self, mock_run_action_input, mock_role):
        ...
        with (
            ...
            patch("allama.executor.activities.ctx_run") as mock_ctx_run,
            patch("allama.executor.activities.ctx_role") as mock_ctx_role,
        ):
            mock_activity.info.return_value = MagicMock(attempt=1)
            mock_backend.return_value = MagicMock()
