...
from allama.db.models import Workflow
from allama.dsl.client import get_temporal_client
from allama.dsl.common import (
    RETRY_POLICIES,
    DSLEntrypoint,
    DSLInput,
    DSLRunArgs,
)
from allama.dsl.enums import (
    JoinStrategy,
    LoopStrategy,
    StreamErrorHandlingStrategy,
    WaitStrategy,
)
from allama.dsl.schemas import (
    ActionStatement,
    DSLConfig,
    ExecutionContext,
    GatherArgs,
...
from allama.secrets.schemas import SecretCreate, SecretKeyValue
from allama.secrets.service import SecretsService
from allama.storage.object import (
    CollectionObject,
    InlineObject,
    StoredObject,
    StoredObjectValidator,
)
from allama.storage.utils import (
    resolve_execution_context as resolve_exec_ctx_from_dict,
)
from allama.tables.enums import SqlType
from allama.tables.schemas import TableColumnCreate, TableCreate, TableRowInsert
from allama.tables.service import TablesService
from allama.variables.schemas import VariableCreate
from allama.variables.service import VariablesService
from allama.workflow.executions.common import unwrap_action_result
from allama.workflow.executions.enums import WorkflowEventType
from allama.workflow.executions.schemas import (
    EventGroup,
    WorkflowExecutionEvent,
)
from allama.workflow.executions.service import WorkflowExecutionsService
from allama.workflow.management.definitions import WorkflowDefinitionsService
from allama.workflow.management.management import WorkflowsManagementService
from allama.workflow.management.schemas import WorkflowUpdate
...
def shutdown_executor_and_worker(monkeysession: pytest.MonkeyPatch):
    ...
    if os.environ.get("GITHUB_ACTIONS") is not None:
        return
    monkeysession.setattr(config, "ALLAMA__API_URL", "http://localhost/api")
        ...
        result = await client.execute_workflow(
            ...
            retry_policy=RetryPolicy(
                ...
                non_retryable_error_types=[
                    "allama.exceptions.AllamaExpressionErrorAllamaValidationError"
                ],
            ),
        )
...
async def test_single_child_workflow_get_correct_secret_environment(
    ...
    # monkeysession.setattr(config, "ALLAMA__UNSAFE_DISABLE_SM_MASKING", True)
...
PARTIAL_DIVISION_BY_ZERO_ERROR = {
    ...
    "message": (
        ...
        "\n"
        "\n"
        "AllamaExpressionError: Error evaluating expression `1/0`\n"
        "\n"
        ...
        with pytest.raises(TemporalError):
            await client.execute_workflow(
                ...
                retry_policy=RetryPolicy(
                    maximum_attempts=1,
                    non_retryable_error_types=[
                        "allama.exceptions.AllamaExpressionError",
...
async def test_workflow_runs_template_for_each(
    ...
    from allama.registry.actions.service import RegistryActionsService
    from allama.registry.repository import Repository
            ...
            "errors": [
                {
                    ...
                    "message": (
                        ...
                        "\n"
                        "------------------------------\n"
                        "File: /app/allama/expressions/core.py\n"
                        "Function: result\n"
                        "Line: 77"
                    ),
                    "ref": "failing_action",
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            "AllamaException: Workflow definition not found for wf_0000000000000000000000, version=None",
            id="id-no-match",
        ),
        pytest.param(
...
async def test_workflow_return_strategy(
    ...
    """
    monkeypatch.setenv("ALLAMA__WORKFLOW_RETURN_STRATEGY", return_strategy)
    monkeypatch.setattr(config, "ALLAMA__WORKFLOW_RETURN_STRATEGY", return_strategy)
...
async def test_workflow_environment_override(
    ...
    monkeypatch.setenv("ALLAMA__WORKFLOW_RETURN_STRATEGY", "context")
    monkeypatch.setattr(config, "ALLAMA__WORKFLOW_RETURN_STRATEGY", "context")
    # Disable secret masking so we can verify the actual secret value
    monkeypatch.setattr(config, "ALLAMA__UNSAFE_DISABLE_SM_MASKING", True)
