...
from allama_ee.agent.activities import BuildToolDefsArgs, BuildToolDefsResult
from allama_ee.agent.approvals.service import (
    ApprovalManager,
    ApprovalMap,
    ApprovalService,
)
...
from allama.agent.common.stream_types import ToolCallContent
...
from allama.agent.session.activities import (
    CreateSessionInput,
    CreateSessionResult,
    LoadSessionInput,
    LoadSessionResult,
...
from allama.db.models import User
from allama.dsl.common import RETRY_POLICIES
from allama.feature_flags import FeatureFlag
...
def create_mock_build_tool_definitions_activity(
    ...
    registry_lock = RegistryLock(
        ...
        actions=dict.fromkeys(tool_definitions.keys(), "allama_registry"),
    )
...
async def test_user(svc_role: Role) -> User:
    ...
    from allama.db.engine import get_async_engine
