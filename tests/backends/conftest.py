...
3. Run benchmarks inside Docker: `make bench`

Note: Sandboxed backends (pool, ephemeral) require nsjail which only
runs on Linux. Use `make bench` to run benchmarks inside Docker on macOS.
"""
...
from allama.dsl.schemas import ExecutionContext
...
def backend_type(
    ...
    monkeypatch.setenv("ALLAMA__EXECUTOR_BACKEND", backend)
    ...
    from allama import config as allama_config

    importlib.reload(allama_config)
...
def direct_backend_type(monkeypatch: pytest.MonkeyPatch) -> str:
    ...
    """
    monkeypatch.setenv("ALLAMA__EXECUTOR_BACKEND", "direct")
...
def require_registry_sync(registry_synced: bool) -> None:
    ...
    if not registry_synced:
        pytest.skip(
            "Registry not synced. Run 'make dev' and sync registry via UI/API first."
        )
...
async def executor_backend(
    ...
    """
    from allama.executor.backends import _create_backend
    ...
    def _create(
        ...
        if registry_lock is None:
            registry_lock = RegistryLock(
                ...
                actions={action: "allama_registry"},
            )
...
def setup_benchmark_environment(monkeypatch_session):
    ...
    if not os.environ.get("ALLAMA__EXECUTOR_BACKEND"):
        monkeypatch_session.setenv("ALLAMA__EXECUTOR_BACKEND", "direct")
...
async def pool_backend() -> AsyncIterator[ExecutorBackend]:
    ...
    from allama.executor.backends.pool import PoolBackend
