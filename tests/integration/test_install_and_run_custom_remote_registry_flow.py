...
6. Run a DSLWorkflow that calls `allama.math.add_300` via Temporal,
...
from allama.dsl.common import DSLEntrypoint, DSLInput, DSLRunArgs
...
from allama.secrets.schemas import SecretCreate
from allama.settings.schemas import GitSettingsUpdate

GIT_SSH_URL = "git+ssh://git@github.com/digitranslab/internal-registry.git"
...
async def test_remote_custom_registry_repo() -> None:
    ...
    2. Execute the `allama.math.add_300` action inside a workflow via Temporal.
    """
    ...
    register_response = session.post(
        ...
        json={"email": "test@allama.com", "password": "password1234"},
    )
    ...
    login_response = session.post(
        ...
        data={"username": "test@allama.com", "password": "password1234"},
    )
    ...
    session.cookies.set("allama-org-id", org_id)
    ...
    action_found = any(
        action["action"] == "allama.math.add_300" for action in actions
    )
    assert action_found, "allama.math.add_300 should be in the repository actions"
    logger.info(
        "Action verification successful",
        action="allama.math.add_300",
    ...
    # (conftest sets config.TEMPORAL__CLUSTER_QUEUE to "test-allama-task-queue" for unit tests,
    # but this integration test runs against the Docker worker which uses the default queue)
    task_queue = "allama-task-queue"
