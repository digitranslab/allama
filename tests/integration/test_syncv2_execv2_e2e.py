...
from allama.db.models import Organization
...
from allama.dsl.worker import new_sandbox_runner
from allama.executor.action_runner import ActionRunner
from allama.executor.backends.ephemeral import EphemeralBackend
from allama.executor.schemas import (
    ActionImplementation,
    ExecutorResultSuccess,
    ResolvedContext,
)
from allama.executor.service import (
    RegistryArtifactsContext,
    get_registry_artifacts_for_lock,
)
from allama.identifiers.workflow import WorkflowUUID
...
from allama.registry.sync.platform_service import PlatformRegistrySyncService
from allama.registry.sync.service import RegistrySyncService
from allama.registry.sync.workflow import (
...
TEST_BUCKET = "test-allama-registry"
...
def subprocess_db_env(monkeypatch):
    ...
    ALLAMA__DB_URI to point to the test database instead of the main
    postgres database.
    ...
    """
    import allama.db.engine as engine_module

    # Convert asyncpg URL to psycopg URL for subprocess
    test_db_url = TEST_DB_CONFIG.test_url.replace("+asyncpg", "+psycopg")
    monkeypatch.setenv("ALLAMA__DB_URI", test_db_url)
    monkeypatch.setattr(config, "ALLAMA__DB_URI", test_db_url)
...
def configure_minio_for_tests(monkeypatch):
    ...
    monkeypatch.setattr(
        config, "ALLAMA__BLOB_STORAGE_ENDPOINT", f"http://{MINIO_ENDPOINT}"
    )
    monkeypatch.setattr(config, "ALLAMA__BLOB_STORAGE_BUCKET_REGISTRY", TEST_BUCKET)
    monkeypatch.setenv("AWS_ACCESS_KEY_ID", AWS_ACCESS_KEY_ID)
    monkeypatch.setenv("AWS_SECRET_ACCESS_KEY", AWS_SECRET_ACCESS_KEY)

    # Disable nsjail for all tests (use subprocess mode for macOS/CI)
    monkeypatch.setenv("ALLAMA__DISABLE_NSJAIL", "true")
    monkeypatch.setattr(config, "ALLAMA__DISABLE_NSJAIL", True)
    monkeypatch.setattr(config, "ALLAMA__EXECUTOR_SANDBOX_ENABLED", False)
    ...
    monkeypatch.setattr(config, "ALLAMA__REGISTRY_SYNC_SANDBOX_ENABLED", False)
...
async def registry_sync_worker(temporal_client):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        async with Worker(
            temporal_client,
            task_queue=config.ALLAMA__EXECUTOR_QUEUE,
...
def run_action_input_factory():
    ...
    def _create(
        ...
        origins = registry_lock or {"allama_registry": "test-version"}
...
def unique_version() -> str:
    ...
    return f"{allama_registry.__version__}-{uuid.uuid4().hex[:8]}"
...
def module_unique_version() -> str:
    ...
    return f"{allama_registry.__version__}-module-{uuid.uuid4().hex[:8]}"
...
async def shared_synced_registry(
    ...
    # Also sync to platform tables (executor queries platform tables for allama_registry)
    platform_svc = PlatformRegistryReposService(module_committing_session)
...
class TestSyncv2MinioIntegration:
    ...
    async def test_sync_builtin_registry_uploads_tarball(
        ...
        This test:
        1. Syncs the builtin allama_registry
...
class TestExecuteWithSyncedRegistry:
    ...
    async def test_execute_action_with_ephemeral_backend(
        ...
        Note: This test runs with ALLAMA__DISABLE_NSJAIL=true to use subprocess
        ...
        input_data = run_action_input_factory(
            ...
            args=args,
            registry_lock={"allama_registry": sync_result.version.version},
        )
        ...
        with (
            patch(
                "allama.executor.backends.ephemeral.get_action_runner",
    ...
    async def test_execute_action_with_registry_lock(
        ...
        with (
            ...
            patch(
                "allama.executor.backends.ephemeral.get_registry_artifacts_for_lock",
...
class TestFailureScenarios:
    ...
    async def test_registry_lock_fails_on_nonexistent_version(
        ...
        registry_lock = {"allama_registry": "nonexistent-version-12345"}
