...
The tests run with ALLAMA__DISABLE_NSJAIL=true, which uses direct subprocess
...
from allama.executor.schemas import ActionImplementation, ResolvedContext
...
def disable_nsjail_for_tests(monkeypatch_session):
    ...
    """
    monkeypatch_session.setenv("ALLAMA__DISABLE_NSJAIL", "true")
    monkeypatch_session.setenv("ALLAMA__POOL_WORKER_TEST_MODE", "true")
...
async def worker_pool():
    ...
    Workers run as direct subprocesses (ALLAMA__DISABLE_NSJAIL=true).
    """
    from allama.executor.backends.pool import WorkerPool
...
def role_workspace_agent_a() -> Role:
    ...
    return Role(
        type="service",
        service_id="allama-agent-executor",
        workspace_id=uuid.UUID("aaaaaaaa-aaaa-4aaa-aaaa-aaaaaaaaaaaa"),
...
def mock_modules_dir(tmp_path: Path) -> Path:
    ...
    """
    # Workspace A module
    ws_a = tmp_path / "workspace_a" / "allama_custom"
...
''')
    ...
    # Workspace B module
    ws_b = tmp_path / "workspace_b" / "allama_custom"
...
def resolved_context_factory():
    ...
    In test mode (ALLAMA__POOL_WORKER_TEST_MODE=true), workers return mock
