...
import pytest
from allama_registry.sdk.workflows import (
...
@pytest.fixture
def workflows_client(mock_allama_client: MagicMock) -> WorkflowsClient:
    """Create a WorkflowsClient with mocked HTTP client."""
    return WorkflowsClient(mock_allama_client)
...
class TestWorkflowsClientExecute:
    ...
    async def test_execute_detach_with_alias(
        self, workflows_client: WorkflowsClient, mock_allama_client: MagicMock
    ):
        """Test execute with detach strategy using alias."""
        mock_allama_client.post.return_value = {
        ...
        mock_allama_client.post.assert_called_once_with(
