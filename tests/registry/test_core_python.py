...
from allama.config import ALLAMA__SANDBOX_PYPI_INDEX_URL
from allama.sandbox import (
    SandboxExecutionError,
    SandboxService,
    SandboxTimeoutError,
    validate_run_python_script,
)
from allama.sandbox.executor import NsjailExecutor
...
class TestPyPIConfiguration:
    ...
    async def test_default_pypi_index_used(self) -> None:
        ...
        assert ALLAMA__SANDBOX_PYPI_INDEX_URL == "https://pypi.org/simple"
    ...
    async def test_custom_pypi_index_configuration(
        ...
        monkeypatch.setenv("ALLAMA__SANDBOX_PYPI_INDEX_URL", custom_url)

        # Reload config to pick up new env var
        importlib.reload(config)

        assert config.ALLAMA__SANDBOX_PYPI_INDEX_URL == custom_url
    ...
    async def test_extra_index_urls_configuration(
        ...
        monkeypatch.setenv("ALLAMA__SANDBOX_PYPI_EXTRA_INDEX_URLS", extra_urls)

        # Reload config to pick up new env var
        importlib.reload(config)

        assert len(config.ALLAMA__SANDBOX_PYPI_EXTRA_INDEX_URLS) == 2
        assert (
            "https://extra1.example.com/simple"
            in config.ALLAMA__SANDBOX_PYPI_EXTRA_INDEX_URLS
        )
        assert (
...
2. Run tests inside the executor container:
   docker exec -it allama-executor-1 pytest tests/registry/test_core_python.py -v

The sandbox requires:
- Linux with kernel >= 4.6 (for user namespaces)
- nsjail binary at /usr/local/bin/nsjail
- Sandbox rootfs at /var/lib/allama/sandbox-rootfs
