...
import pytest
import allama_registry.core.cases as cases_core
...
class TestCoreListCases:
    ...
    async def test_list_cases_limit_validation(self, mock_cases_client: AsyncMock):
        ...
        from allama_registry import config
...
class TestCoreAttachments:
    ...
    async def test_upload_attachment_invalid_base64(
        ...
        with pytest.raises(AllamaValidationError, match="Invalid base64 encoding"):
    ...
    async def test_get_attachment_download_url_invalid_expiry(
        ...
        with pytest.raises(
            AllamaValidationError, match="Expiry must be a positive number"
        ):
    ...
    async def test_get_attachment_download_url_expiry_too_long(
        ...
        with pytest.raises(
            AllamaValidationError, match="Expiry cannot exceed 24 hours"
        ):
            await get_attachment_download_url(
                case_id=case_id,
                attachment_id=attachment_id,
...
class TestCoreUploadAttachmentFromURL:
    ...
    @patch("allama_registry.core.cases._upload_attachment", new_callable=AsyncMock)
    @patch("allama_registry.core.cases.httpx.AsyncClient")
    ...
    async def test_upload_attachment_from_url_empty_content(
        ...
        case_id = mock_case_dict["id"]
        with pytest.raises(AllamaValidationError, match="No content found"):
    ...
    async def test_upload_attachment_from_url_no_content_type(
        ...
        case_id = mock_case_dict["id"]
        with pytest.raises(AllamaValidationError, match="No content type found"):
            await upload_attachment_from_url(
                case_id=case_id,
                url="https://example.com/docs/report.pdf",
            )
