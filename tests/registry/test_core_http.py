...
from allama_registry._internal.exceptions import AllamaException
from allama_registry.core.http import (
    FileUploadData,
    http_paginate,
    http_poll,
    http_request,
...
from allama.expressions.functions import str_to_b64
...
async def test_http_request_failure() -> None:
    ...
    with pytest.raises(AllamaException) as excinfo:
        await http_request(
            url="https://api.example.com",
            method="GET",
        )
...
async def test_http_request_files_missing_content_base64_in_dict() -> None:
    ...
    with pytest.raises(
        AllamaException, match=r"Missing 'content_base64' for form field 'data_file'"
    ):
        await http_request(
            url="https://api.example.com",
            method="POST",
...
async def test_http_request_files_empty_form_field_name() -> None:
    ...
    base64_content = base64.b64encode(b"test").decode("utf-8")
    with pytest.raises(
        AllamaException, match=r"Invalid form_field_name.*cannot be empty"
    ):
        await http_request(
...
async def test_http_request_with_invalid_base64_in_files() -> None:
    ...
    with pytest.raises(
        AllamaException, match=r"Invalid base64 data for form field 'test.txt'"
    ):
        await http_request(
            url="https://api.example.com",
            method="POST",
...
async def test_http_request_file_upload_max_files_exceeded() -> None:
    """Test HTTP request fails if number of files exceeds ALLAMA__MAX_UPLOAD_FILES_COUNT."""
    from allama_registry.core import http as core_http  # For accessing the constant

    files_to_upload: dict[str, str | FileUploadData] = {}
    for i in range(core_http.ALLAMA__MAX_UPLOAD_FILES_COUNT + 1):
    ...
    assert (
        f"Number of files ({core_http.ALLAMA__MAX_UPLOAD_FILES_COUNT + 1}) exceeds the maximum allowed limit of {core_http.ALLAMA__MAX_UPLOAD_FILES_COUNT}"
...
async def test_http_request_file_upload_aggregate_size_exceeded() -> None:
    """Test HTTP request fails if total size of files exceeds ALLAMA__MAX_AGGREGATE_UPLOAD_SIZE_BYTES."""
    pytest.skip(
