...
from allama_registry._internal import secrets as registry_secrets
from allama_registry.core.sql import (
...
def test_validate_connection_url_blocks_internal_endpoint(
    ...
    from allama_registry import config as registry_config

    monkeypatch.setattr(
        registry_config,
        "ALLAMA__DB_URI",
        "postgresql+psycopg://allama:secret@internal-db:5432/allama",
    )
    monkeypatch.setattr(registry_config, "ALLAMA__DB_ENDPOINT", "internal-db")
    monkeypatch.setattr(registry_config, "ALLAMA__DB_PORT", "5432")
...
def test_validate_connection_url_uses_internal_uri_fallback(
    ...
    """Fallback to ALLAMA__DB_URI when ALLAMA__DB_ENDPOINT is unset."""
    from allama_registry import config as registry_config

    monkeypatch.setattr(
        registry_config,
        "ALLAMA__DB_URI",
        "postgresql+psycopg://allama:secret@postgres_db:6432/allama",
    )
    monkeypatch.setattr(registry_config, "ALLAMA__DB_ENDPOINT", None)
    monkeypatch.setattr(registry_config, "ALLAMA__DB_PORT", None)
