...
from allama_registry.context import RegistryContext, clear_context, set_context
from allama_registry.core.cases import (
    add_case_tag,
    assign_user,
...
from allama_registry.core.ee.durations import get_case_metrics
from allama_registry.core.ee.tasks import create_task, list_tasks
from allama_registry.sdk.exceptions import (
    AllamaNotFoundError as SDKNotFoundError,
)
from allama_registry.sdk.exceptions import (
    AllamaValidationError as SDKValidationError,
)

from allama import config
from allama.api.app import app
from allama.auth.dependencies import (
...
from allama.cases.durations.schemas import (
    CaseDurationDefinitionCreate,
    CaseDurationEventAnchor,
)
from allama.cases.durations.service import CaseDurationDefinitionService
from allama.cases.enums import CaseEventType
from allama.cases.router import WorkspaceAdminUser, WorkspaceUser
from allama.cases.service import CaseFieldsService
from allama.contexts import ctx_role
from allama.db.dependencies import get_async_session
from allama.db.models import User, Workspace
...
_CASE_FIELDS_SCHEMA_LOCK_ID = 0x7472616365636174  # "allama" in hex
...
async def cases_ctx(
    ...
    registry_ctx = RegistryContext(
        ...
        run_id="test-run-id",
        environment="default",
        api_url=config.ALLAMA__API_URL,
    )
    ...
    respx_mock.start()
    respx_mock.route(url__startswith=config.ALLAMA__API_URL).mock(
...
def blob_storage_config(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setenv("ALLAMA__BLOB_STORAGE_ENDPOINT", "http://localhost:9000")
    monkeypatch.setenv("AWS_ACCESS_KEY_ID", "minioadmin")
    monkeypatch.setenv("AWS_SECRET_ACCESS_KEY", "minioadmin")

    # Also update the config module's values directly since they're already loaded
    monkeypatch.setattr(
        config, "ALLAMA__BLOB_STORAGE_ENDPOINT", "http://localhost:9000"
    )
...
class TestGetAttachmentDownloadUrl:
    ...
    async def test_get_attachment_download_url_returns_url(
        ...
        assert "allama-attachments" in result or "attachments" in result
