...
from allama_registry.integrations.ldap3 import search_entries
...
def ldap_test_data(live_ldap_server: dict[str, Any]) -> dict[str, str]:
    ...
    conn.add(
        ...
        ["inetOrgPerson", "organizationalPerson", "person", "top"],
        {
            ...
            "sn": "Test",
            "uid": "allama.test",
            "mail": "allama@example.com",
        },
    )
    conn.unbind()
    return {"base_dn": users_dn, "filter": "(uid=allama.test)"}
...
def test_live_search_all_attributes(
    ...
    assert results and results[0]["attributes"]["uid"] == ["allama.test"]
...
def test_live_search_returns_all_attributes(
    ...
    assert "cn" in attrs
    assert attrs["uid"] == ["allama.test"]
    assert attrs["mail"] == ["allama@example.com"]
