...
import pytest
from allama_registry import ActionIsInterfaceError
from allama_registry.core.agent import action, agent
...
async def test_action_primitives(output_type: Any) -> None:
    ...
    with patch("allama_registry.core.agent.get_context", return_value=mock_ctx):
        result = await action(
            user_prompt=user_prompt,
            model_name="gpt-4o-mini",
            model_provider="openai",
