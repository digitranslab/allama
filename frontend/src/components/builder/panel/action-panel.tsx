...
import type { RequestValidationError, AllamaApiError } from "@/lib/errors"
...
import { isAllamaJsonSchema, type AllamaJsonSchema } from "@/lib/schema"
                      ...
                      {inputMode === "form" && formModeEnabled && (
                        <>
                          ...
                          {requiredFields.map(([fieldName, fieldDefn]) => {
                            ...
                            if (!isAllamaJsonSchema(fieldDefn)) {
                              // For non-AllamaJsonSchema, we can't extract type information
                              return (
                                <ControlledYamlField
                                  key={fieldName}
                                  label={label}
                                ...
                                >
                                  {optionalFields.map(
                                    ([fieldName, fieldDefn]) => {
                                      ...
                                      const isVisible =
                                        visibleOptionalFields.has(fieldName)
                                      const description = isAllamaJsonSchema(
                                        fieldDefn
                                      )
