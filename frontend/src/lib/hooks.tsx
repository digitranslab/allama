...
import { retryHandler, type AllamaApiError } from "@/lib/errors"
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        throw new Error(
          "Unable to fetch authentication settings. This could be a network issue with the Allama API."
        )
      }
    },
  })
...
export function useWorkspaceManager() {
  ...
  const getLastWorkspaceId = useCallback(
    () => Cookies.get("__allama:workspaces:last-viewed"),
    []
  )
  const setLastWorkspaceId = useCallback((id?: string) => {
    if (!id) {
      Cookies.set("__allama:workspaces:last-viewed", "")
      return
    }
    Cookies.set("__allama:workspaces:last-viewed", id)
  }, [])
  const clearLastWorkspaceId = useCallback(() => {
    Cookies.remove("__allama:workspaces:last-viewed")
  }, [])
...
export function useCreateManualWorkflowExecution(workflowId: string) {
  ...
  const {
    ...
    onError: (error: AllamaApiError<Record<string, string>>) => {
      switch (error.status) {
        case 400:
...
export function useLastExecution({
  ...
  } = useQuery<WorkflowExecutionReadMinimal | null, AllamaApiError>({
    enabled: !!workflowId,
...
export function useRegistryActions(versions?: string[]) {
  ...
  const {
    ...
    onError: (error: AllamaApiError) => {
      ...
      const apiError = error as AllamaApiError
...
export function useOrgAuditSettings() {
  ...
  } = useMutation<AuditApiKeyGenerateResponse, AllamaApiError>({
  ...
  } = useMutation<void, AllamaApiError>({
...
export function useUpdateColumn() {
  ...
  const {
    ...
    onError: (error: AllamaApiError, variables) => {
...
export function useListCases({ workspaceId }: CasesListCasesData) {
  const {
    ...
    error: casesError,
  } = useQuery<CaseReadMinimal[], AllamaApiError>({
...
export function useGetCase(
  ...
  const {
    ...
    error: caseDataError,
  } = useQuery<CaseRead, AllamaApiError>({
...
export function useCaseDurations({
  ...
  } = useQuery<CaseDurationRead[], AllamaApiError>({
...
export function useCaseFields(workspaceId: string) {
  ...
  } = useQuery<CaseFieldReadMinimal[], AllamaApiError>({
...
export function useCaseComments({
  ...
  } = useQuery<CaseCommentRead[], AllamaApiError>({
...
export function useCaseTasks({ caseId, workspaceId }: CasesListTasksData) {
  ...
  } = useQuery<CaseTaskRead[], AllamaApiError>({
...
export function useCaseEvents({
  ...
  } = useQuery<CaseEventsWithUsers, AllamaApiError>({
    queryKey: ["case-events", caseId, workspaceId],
    queryFn: async () =>
...
export function useCaseTags({ caseId, workspaceId }: CasesListTagsData) {
  const {
    ...
    isLoading: caseTagsIsLoading,
    error: caseTagsError,
  } = useQuery<CaseTagRead[], AllamaApiError>({
...
export function useIntegrations(workspaceId: string) {
  ...
  } = useQuery<IntegrationReadMinimal[], AllamaApiError>({
  ...
  } = useQuery<ProviderReadMinimal[], AllamaApiError>({
...
export function useListMcpIntegrations(workspaceId: string) {
  ...
  } = useQuery<MCPIntegrationRead[], AllamaApiError>({
    queryKey: ["mcp-integrations", workspaceId],
    queryFn: async () =>
...
export function useIntegrationProvider({
  ...
  } = useQuery<IntegrationRead, AllamaApiError>({
  ...
  const {
    ...
    error: providerError,
  } = useQuery<ProviderRead, AllamaApiError>({
