...
import {
  ALLAMA_COMPONENT_KEY,
  type AllamaJsonSchema,
  type AllamaJsonSchemaDefinition,
  type AllamaJsonSchemaType,
} from "@/lib/schema"

export interface JSONSchemaParam {
  parameter: string
  type: string
  default: AllamaJsonSchemaType
  description: string
  constraints: string
  required: boolean
}
...
export function jsonSchemaToParams(
  ...
  const rows: JSONSchemaParam[] = Object.entries(properties).map(
    ([key, value]) => {
      const jsonSchemaValue = value as AllamaJsonSchema
...
export function getType(value: AllamaJsonSchema): string {
  // Handle anyOf
  if (value.anyOf) {
...
function getTypeFromDefinition(
  definition: AllamaJsonSchemaDefinition
): string {
...
export function getConstraints(value: AllamaJsonSchema): string {
  const constraints = []
  ...
  for (const key in value) {
    ...
    const cons = value[key as keyof AllamaJsonSchema]
    if (key === "additionalProperties") {
      constraints.push(
        `additionalProperties: ${getType(cons as AllamaJsonSchema)}`
      )
...
export function jsonSchemaToZod(schema: JSONSchema7): z.ZodTypeAny {
  ...
  if (schema.type) {
    switch (schema.type) {
      ...
      case "array":
        if (schema.items) {
          const itemSchema = jsonSchemaToZod(schema.items as AllamaJsonSchema)
      ...
      case "object":
        if (schema.properties) {
          ...
          Object.entries(schema.properties).forEach(([key, prop]) => {
            ...
            let fieldSchema = jsonSchemaToZod(prop as AllamaJsonSchema)
