...
from allama.auth.types import PlatformRole
from allama.db.models import PlatformSetting
from allama.secrets.encryption import decrypt_value, encrypt_value
from allama.service import BasePlatformService
from allama_ee.admin.settings.schemas import (
...
class AdminSettingsService(BasePlatformService):
    ...
    def __init__(self, session: AsyncSession, role: PlatformRole):
        ...
        try:
            self._encryption_key = SecretStr(os.environ["ALLAMA__DB_ENCRYPTION_KEY"])
        except KeyError as e:
            raise KeyError("ALLAMA__DB_ENCRYPTION_KEY is not set") from e
