...
import orjson
from allama_registry.integrations.agents.prompts.slackbot import (
    SlackAppMentionPrompts,
    SlackInteractionPrompts,
    SlackNoEventPrompts,
    SlackPrompts,
)
from allama_registry.integrations.slack_sdk import call_method, slack_secret
from allama_registry.sdk.agents import run_agent

from allama_registry import registry
from allama_registry.core.agent import PYDANTIC_AI_REGISTRY_SECRETS
from allama_registry.fields import ActionType, TextArea
...
async def slackbot(
    event: Annotated[
        dict[str, Any] | None,
        Doc(
            "Slack app mention event or interaction payload (e.g. for button clicks) passed in via Allama webhook TRIGGER. If None, the agent will send a message to the channel."
        ),
    ],
    prompt: Annotated[
        str,
