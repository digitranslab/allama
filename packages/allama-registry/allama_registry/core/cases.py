...
from allama_registry import config, registry, types
...
def _infer_filename_from_url(url: str) -> str:
    ...
    raise AllamaValidationError(detail=f"Unable to infer filename from URL: {url}")
...
async def download_attachment(
    ...
    try:
        ...
        raise AllamaValidationError(detail=f"Invalid ID format: {str(e)}") from e
