...
from allama_registry import RegistrySecret, registry, secrets
from allama_registry._internal.exceptions import AllamaException
from allama_registry._internal.jsonpath import eval_jsonpath
from allama_registry._internal.logger import logger
from allama_registry._internal.safe_lambda import build_safe_lambda
from allama_registry.config import (
    ALLAMA__MAX_AGGREGATE_UPLOAD_SIZE_BYTES,
    ALLAMA__MAX_FILE_SIZE_BYTES,
    ALLAMA__MAX_UPLOAD_FILES_COUNT,
)
...
def _validate_and_decode_file(
    ...
    if len(decoded_content) > ALLAMA__MAX_FILE_SIZE_BYTES:
        raise ValueError(
            f"File for form field '{form_field_name}' in {action_name} exceeds maximum size limit of "
            f"{ALLAMA__MAX_FILE_SIZE_BYTES // 1024 // 1024}MB."
        )
...
def _process_file_uploads(
    ...
    if len(files) > ALLAMA__MAX_UPLOAD_FILES_COUNT:
        raise ValueError(
            f"Number of files ({len(files)}) exceeds the maximum allowed limit "
            f"of {ALLAMA__MAX_UPLOAD_FILES_COUNT} in {action_name}."
        )
    ...
    for form_field_name, file_input in files.items():
        ...
        if current_aggregate_size > ALLAMA__MAX_AGGREGATE_UPLOAD_SIZE_BYTES:
            raise ValueError(
                f"Total size of files ({current_aggregate_size / 1024 / 1024:.2f}MB) "
                f"exceeds the aggregate limit of "
                f"{ALLAMA__MAX_AGGREGATE_UPLOAD_SIZE_BYTES / 1024 / 1024:.2f}MB "
                f"in {action_name}."
            )
...
async def http_request(
    ...
    try:
        ...
        raise AllamaException(str(e)) from e
    ...
    with (
        ...
        try:
            ...
            raise AllamaException(error_message) from e
    ...
    *,
    ...
    while len(results) < limit:
        ...
        if items_jsonpath is not None:
            ...
            # a AllamaExpressionError, surfacing misuse clearly to users.
            if isinstance(data, dict):
