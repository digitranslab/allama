...
from allama_registry import RegistrySecret, config, registry, secrets


# Maximum number of rows to return from a query
# This can be overridden via ALLAMA__MAX_ROWS_CLIENT_POSTGRES env var
...
def _validate_connection_url(connection_url: URL) -> None:
    """Validate that the connection URL does not point to Allama's internal database.
    ...
    Raises:
        SQLConnectionValidationError: If connection attempts to access Allama's database
    """
    # Parse internal database URL
    try:
        internal_url = make_url(config.ALLAMA__DB_URI)
    ...
    internal_host = config.ALLAMA__DB_ENDPOINT or internal_url.host
    ...
    try:
        internal_port = (
            int(config.ALLAMA__DB_PORT)
            if config.ALLAMA__DB_PORT is not None
            else None
        )
    ...
    if connection_url.host:
        ...
        if hmac.compare_digest(user_host, internal_host_lower):
            ...
            if user_port == internal_port:
                raise SQLConnectionValidationError(
                    "Cannot connect to Allama's internal database endpoint. Use an external database connection URL instead."
                )
...
async def execute_query(
    query: Annotated[
        ...
        Doc(
            ...
            "Do NOT use Allama expressions in the query string.",
        ),
    ],
