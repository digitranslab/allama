...
from allama_registry import types as registry_types
...
class AgentConfig:
    ...
    retries: int = config.ALLAMA__AGENT_MAX_RETRIES
...
async def run_agent(
    ...
    output_type: OutputType | None = None,
    model_settings: dict[str, object] | None = None,
    max_tool_calls: int = config.ALLAMA__AGENT_MAX_TOOL_CALLS,
    max_requests: int = config.ALLAMA__AGENT_MAX_REQUESTS,
    retries: int = config.ALLAMA__AGENT_MAX_RETRIES,
    base_url: str | None = None,
...
class AgentsClient:
    ...
    async def get_preset(self, slug: str) -> dict[str, Any]:
        ...
        Raises:
            AllamaNotFoundError: If preset doesn't exist.
        """
