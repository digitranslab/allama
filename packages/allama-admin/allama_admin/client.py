"""HTTP client for Allama Admin API."""
...
from allama_admin.config import Config, get_config, load_cookies
from allama_admin.schemas import (
...
class AdminClient:
    """Async HTTP client for Allama Admin API.

    Uses service key authentication via x-allama-service-key header.
    """
    ...
    async def __aenter__(self) -> AdminClient:
        ...
        if cookies:
            ...
            self._client = httpx.AsyncClient(
                base_url=self._config.api_url,
                headers={
                    "x-allama-service-key": self._config.service_key,
                    "x-allama-role-service-id": "allama-admin-cli",
            ...
            raise AdminClientError(
                "Not authenticated. Run 'allama auth login' or set ALLAMA__SERVICE_KEY"
            )
        return self
