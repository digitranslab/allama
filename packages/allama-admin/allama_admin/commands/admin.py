...
from allama_admin.output import (
    print_error,
    print_success,
    print_user_detail,
    print_users_table,
...
async def promote_user(
    ...
    Requires: ALLAMA__SERVICE_KEY environment variable.
    """
    try:
        async with AdminClient() as client:
...
def create_superuser(
    ...
    Requires: ALLAMA__DB_URI environment variable or allama[bootstrap] installed.
    """
    try:
        from allama_admin.services.bootstrap import (
            create_superuser as bootstrap_create_superuser,
        )
    except ImportError:
        print_error(
            "Bootstrap dependencies not installed. "
            "Install with: pip install allama-admin[bootstrap]"
        )
        raise typer.Exit(1) from None
