...
from allama_admin.config import get_config
from allama_admin.output import print_error, print_success, print_warning

app = typer.Typer(no_args_is_help=True)


def _get_alembic_dir() -> str | None:
    """Find the alembic directory relative to allama installation."""
    try:
        import allama

        allama_dir = os.path.dirname(allama.__file__)
        # Alembic is typically at the repo root, one level up from allama package
        repo_root = os.path.dirname(allama_dir)
...
def _run_alembic(args: list[str], db_uri: str | None = None) -> int:
    ...
    if not effective_db_uri:
        print_error(
            "ALLAMA__DB_URI environment variable is required for migration commands"
        )
        return 1
    ...
    if not alembic_dir:
        print_error(
            ...
            "Ensure allama is installed and alembic/ exists."
        )
        return 1

    env = os.environ.copy()
    env["ALLAMA__DB_URI"] = effective_db_uri
...
def upgrade(
    ...
    db_uri: Annotated[
        str | None,
        typer.Option("--db-uri", help="Database URI (overrides ALLAMA__DB_URI)"),
    ] = None,
) -> None:
    """Upgrade database to a specific revision.

    Examples:
        allama migrate upgrade head      # Upgrade to latest
        allama migrate upgrade +1        # Upgrade one revision
        allama migrate upgrade abc123    # Upgrade to specific revision
    """
...
def downgrade(
    ...
    Examples:
        allama migrate downgrade -1      # Downgrade one revision
        allama migrate downgrade base    # Downgrade to initial state
        allama migrate downgrade abc123  # Downgrade to specific revision
    """
    if not yes:
