...
## Project Overview
Allama is a modern, open source automation platform built for security and IT engineers. No-code UI workflows, built-in lookup tables, case management, and Temporal orchestration.
...
# (includes both allama and allama_registry packages)
uv sync
...
# Start with auto-registered test user (test@allama.com / password1234)
...
uv run basedpyright allama/api/
...
- UV lock sync
- Frontend client generation (when allama/packages change)
...
### Core Components
- **API Service** (`allama/api/`): FastAPI application with auth, workflows, cases
- **Worker Service** (`allama/dsl/worker.py`): Temporal workflow worker
- **Executor Service** (`allama/executor/`): Action execution engine with index-based registry resolution
- **Agent System** (`allama/agent/`): LLM-powered agents with multi-runtime support (PydanticAI, Claude Code), MCP integration, tool execution
- **Organization Service** (`allama/organization/`): Multi-tenancy and organization membership management
- **allama-admin CLI** (`packages/allama-admin/`): CLI tool for platform operators (admin, auth, migrate, orgs, registry commands)
- **Frontend** (`frontend/`): Next.js 15 with TypeScript, React Query, Tailwind CSS
- **Registry** (`packages/allama-registry/`): Independent package for integrations and templates
...
- **Database Models**: `allama/db/models.py` - SQLAlchemy database tables. Never add methods here, keep imports minimal
- **Migrations**: `alembic/` directory with comprehensive schema evolution
- **Database Engine**: `allama/db/engine.py` for connection management
...
1. **`models.py`**: Database models (SQLAlchemy tables)
   - Location: `allama/db/models.py`
   - Purpose: Database table definitions
   - Rules: Never add methods, keep imports minimal

2. **`schemas.py`**: API request/response schemas (Pydantic models)
   - Location: Throughout codebase (e.g., `allama/agent/schemas.py`, `allama/workflow/management/schemas.py`)
...
3. **`types.py`**: Domain types, protocols, and type aliases
   - Location: Throughout codebase (e.g., `allama/agent/types.py`, `allama/workflow/management/types.py`)
   - Purpose: Service-level types, protocols, dataclasses, type aliases
   - Use: Domain logic types that don't fit in schemas or models

### Enterprise Edition
- **Package**: `packages/allama-ee/` contains paid enterprise features
- **Installation**: Install with `uv sync` or `pip install allama[ee]`
- **Shims**: `allama/ee/` contains shims for backward compatibility
- **Features**: RBAC, multi-tenancy, SSO integration, advanced auth, interactions

### Agent System
- **Location**: `allama/agent/`
...
Example structure for a module like `allama/agent/`:
```
allama/agent/
...
Services inherit from `BaseService` (`allama/service.py`) which provides:
...
Context variables (`allama/contexts.py`) for request-scoped state:
...
Router access control using predefined roles from `allama/auth/dependencies.py`:
```python
from allama.auth.dependencies import WorkspaceUserRole, OrgAdminUser
...
- `allama/service.py`: Base service class with context-aware defaults
- `allama/contexts.py`: Context variables for request-scoped state
...
- **Parallel testing**: pytest-xdist support with worker-specific:
  - Temporal task queues: `allama-task-queue-{worker_id}`
  - Redis databases: Worker offset % 16
  - Port configuration via environment variables

### Action Templates and Registry
- **Templates**: `packages/allama-registry/allama_registry/templates/` - YAML-based integration templates
- **Integrations**: `packages/allama-registry/allama_registry/integrations/` - Python client integrations
- **Reference file**: `allama/expressions/expectations.py` – Source of primitive type mappings (e.g., `str`, `int`, `Any`) used when defining `expects:` sections in templates.
...
- **DSL**: `allama/dsl/` - Domain Specific Language for workflows
- **Executor**: `allama/executor/` - Action execution engine with Ray distributed computing
- **Temporal**: Workflow orchestration with `allama/dsl/worker.py`

## Important Rules
- Never add methods in `allama/db/models.py`. Keep imports minimal.
...
- When running an alembic migration, first check the PostgreSQL port with `just cluster ports`, then set the DB URI:
  ```bash
  export ALLAMA__DB_URI=postgresql+psycopg://postgres:postgres@localhost:<port>/postgres
  ```
...
## Allama Template Best Practices

### Template Structure
- Templates are YAML files located in `packages/allama-registry/allama_registry/templates/`
...
- String casting: `str()` is valid in Allama templates
- All FN. functions must exist in `allama/expressions/functions.py`

### Available Functions (FN.)
IMPORTANT: Always check `@allama/expressions/functions.py` for the complete and up-to-date list of available functions. Look at the `_FUNCTION_MAPPING` dictionary (around line 905) to see all available functions and their aliases.
