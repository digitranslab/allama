...
This Helm chart deploys Allama on Kubernetes with support for both internal subcharts (CloudNativePG, Valkey, MinIO, Temporal) and external managed services (RDS, ElastiCache, S3).
...
```bash
kubectl create namespace allama
```
...
kubectl create secret generic allama-secrets \
  -n allama \
...
```bash
helm install allama ./allama \
  -n allama \
  --set secrets.existingSecret=allama-secrets \
  --set allama.auth.superadminEmail=admin@example.com \
  --set ingress.host=allama.example.com
```
...
**Step 4: Access Allama**

Once all pods are running, access the UI at: **http://allama.k8s.orb.local**
...
| `allama.auth.superadminEmail` | Initial admin email (required on first install) |
...
| `ingress.host` | allama.example.com | Hostname |
...
```yaml
urls:
  publicApp: https://allama.example.com
  publicApi: https://allama.example.com/api
```
...
| Parameter | Default | Description |
|-----------|---------|-------------|
| `allama.logLevel` | INFO | LOG_LEVEL for backend services |
| `allama.featureFlags` | `""` | Core flags (comma-separated) |
| `enterprise.featureFlags` | "" | Enterprise-only flags appended to `allama.featureFlags` |
...
The chart runs a Helm hook job after install/upgrade to create the `default` namespace and the search attributes in `allama.temporal.searchAttributes`. Workers wait for this setup before starting.
...
externalSecrets:
  ...
  coreSecrets:
    enabled: true
    secretArn: "arn:aws:secretsmanager:us-east-1:123456789:secret:allama/core-AbCdEf"
    targetSecretName: "allama-secrets"

  # Optional: PostgreSQL, Redis, Temporal credentials
  postgres:
    enabled: true
    secretArn: "arn:aws:secretsmanager:us-east-1:123456789:secret:rds!db-xxxx"
    targetSecretName: "allama-postgres-credentials"
```
...
```bash
helm upgrade allama ./allama -n allama -f my-values.yaml
```

## Uninstalling

```bash
helm uninstall allama -n allama
```
