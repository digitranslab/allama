# Allama EKS Terraform stack

Deploy Allama on AWS EKS with managed services (RDS PostgreSQL, ElastiCache Redis, S3).

## Prerequisites

- Terraform
- `openssl` to create cryptographic keys used in the Allama app
...
- S3 access is limited to the Allama IRSA role via bucket policies; pods must use the chart’s service account.
...
export DOMAIN_NAME="allama.example.com"
...
# 2. Create Allama secret in AWS Secrets Manager
aws secretsmanager create-secret --name allama/secrets \
  --secret-string '{
...
aws secretsmanager create-secret --name allama/temporal-api-key \
  --secret-string "your-temporal-cloud-api-key"

# 4. Store secret ARNs in variables
allama_secrets_arn=$(aws secretsmanager describe-secret --secret-id allama/secrets | jq -r '.ARN')
temporal_cloud_api_key_secret_arn=""
if aws secretsmanager describe-secret --secret-id allama/temporal-api-key >/dev/null 2>&1; then
  temporal_cloud_api_key_secret_arn=$(aws secretsmanager describe-secret --secret-id allama/temporal-api-key | jq -r '.ARN')
fi

echo "Allama secrets ARN: $allama_secrets_arn"
...
# 5. Run Terraform to deploy Allama
export TF_VAR_allama_secrets_arn=$allama_secrets_arn
