...
resource "aws_security_group" "rds" {
  ...
  ingress {
    ...
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [aws_security_group.allama_postgres_client.id]
    description     = "PostgreSQL from Allama pods with SecurityGroupPolicy"
  }
...
resource "aws_security_group" "allama_postgres_client" {
  name        = "${var.cluster_name}-postgres-client-sg"
  description = "Pod security group for Allama PostgreSQL access"
  vpc_id      = var.vpc_id
  ...
  ingress {
    ...
    protocol    = "-1"
    cidr_blocks = [data.aws_vpc.selected.cidr_block]
    description = "Allow intra-VPC traffic to Allama pods"
  }
...
# RDS Subnet Group
resource "aws_db_subnet_group" "allama" {
...
resource "aws_db_instance" "allama" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.allama.name
...
resource "null_resource" "create_temporal_databases" {
  ...
  triggers = {
    rds_address = aws_db_instance.allama.address
  ...
  provisioner "local-exec" {
    command = <<-EOT
      set -euo pipefail
      kubectl delete job temporal-db-setup -n ${kubernetes_namespace.allama.metadata[0].name} --ignore-not-found
    ...
    spec:
      ...
      - name: db-setup
        ...
        - name: PGPASSWORD
          valueFrom:
            secretKeyRef:
              name: allama-postgres-credentials
              key: password
        command:
        - /bin/sh
        - -c
...
EOF
      ...
      if ! kubectl wait --for=condition=complete job/temporal-db-setup -n ${kubernetes_namespace.allama.metadata[0].name} --timeout=120s; then
        echo "temporal-db-setup job did not complete; dumping logs for debugging"
        kubectl logs job/temporal-db-setup -n ${kubernetes_namespace.allama.metadata[0].name} || true
        kubectl describe job/temporal-db-setup -n ${kubernetes_namespace.allama.metadata[0].name} || true
        exit 1
      fi
    EOT
  }

  depends_on = [aws_db_instance.allama, null_resource.postgres_credentials_external_secret, null_resource.allama_postgres_sg_policy, kubernetes_namespace.allama]
}
