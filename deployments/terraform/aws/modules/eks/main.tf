...
locals {
  ...
  rds_master_secret_arn = aws_db_instance.allama.master_user_secret[0].secret_arn

  # Secrets synced by ESO
  external_secrets_secret_arns = compact([
    var.allama_secrets_arn,
  ...
  external_secrets_store_name = "allama-aws-secrets"

  # Allama service account name for IRSA (matches Helm release defaults)
  allama_service_account_name = "allama-app"

  # S3 bucket names (using random suffix instead of account ID for security)
  s3_suffix             = random_id.s3_suffix.hex
  s3_attachments_bucket = "allama-attachments-${local.s3_suffix}"
  s3_registry_bucket    = "allama-registry-${local.s3_suffix}"
