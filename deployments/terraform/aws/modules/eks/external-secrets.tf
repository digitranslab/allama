...
# The Allama Helm chart creates ExternalSecret resources for all needed secrets.
...
# Note: Most ExternalSecret resources are created by the Allama Helm chart.
# The chart's externalSecrets.* configuration creates ExternalSecrets for:
# - Core Allama secrets (dbEncryptionKey, serviceKey, signingSecret, userAuthSecret)
...
kind: ExternalSecret
metadata:
  name: allama-postgres-credentials-pre
  namespace: ${kubernetes_namespace.allama.metadata[0].name}
  labels:
    app.kubernetes.io/managed-by: terraform
    app.kubernetes.io/part-of: allama
...
EOF
      ...
      for i in $(seq 1 12); do
        if kubectl get secret allama-postgres-credentials -n ${kubernetes_namespace.allama.metadata[0].name} >/dev/null 2>&1; then
          echo "Secret allama-postgres-credentials created successfully"
          exit 0
        fi
  ...
  provisioner "local-exec" {
    when    = destroy
    command = "kubectl delete externalsecret allama-postgres-credentials-pre -n ${self.triggers.namespace} --ignore-not-found"
  }

  depends_on = [null_resource.external_secrets_cluster_store, kubernetes_namespace.allama]
}
