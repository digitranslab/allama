...
variable "domain_name" {
  description = "Domain name for Allama (e.g., allama.example.com)"
  type        = string
}
...
# Allama Configuration
variable "allama_version" {
  description = "Version of the Allama Helm chart to deploy"
  type        = string
  default     = ""
}

variable "allama_image_tag" {
  description = "Docker image tag for Allama services"
  type        = string
  default     = "latest"
}

variable "superadmin_email" {
  description = "Email address for the Allama superadmin"
  type        = string
}

variable "allama_secrets_arn" {
  description = "ARN of AWS Secrets Manager secret containing Allama secrets (JSON with keys: dbEncryptionKey, serviceKey, signingSecret, userAuthSecret)"
  type        = string
}
